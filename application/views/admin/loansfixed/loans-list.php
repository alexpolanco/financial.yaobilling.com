...
?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>Préstamos interes fijo <?php echo empty($loansdue) ? 'activos' : $loansdue ; ?> (<?php echo count($recored); ?>)</h1>
      <div class="breadcrumb">
        <?php if($loans_origen == 'due') : ?>
        <a id="loans" href="<?php echo base_url().'loansfixed'; ?>" class="btn btn-lg btn-success">Préstamos activos</a>
        <?php else : ?>
        <a id="loans_due" href="<?php echo base_url().'loansfixed/due'; ?>" class="btn btn-lg btn-danger">Préstamos saldados</a>
				<?php endif; ?>
        <!-- <a id="receipts" href="<?php echo base_url().'loansfixed/receipt'; ?>" class="btn btn-lg btn-success">Recibos</a> -->
        <a id="new_loan" href="<?php echo base_url().'loansfixed/create'; ?>" class="btn btn-lg btn-primary">Nuevo Préstamo</a> 
      </div>
    </section>
        ...
        <div class="col-md-12">
            <div class="box box-primary">
              ...
              <?php 
                ...
                echo form_open('loansfixed/delete',$attributes) ?>
                <table id="example" class="display table cell-border row-border hover" style="width:100%">
                  ...
                  <tr data-child-value="<?php echo $_recored->loans_no; ?>" data-child-name="<?php echo  $_recored->customer_first_name; ?>" data-child-loans_amount="<?php echo $_recored->loans_amount; ?>"  data-child-current_balance="<?php echo $loanstotal_capital; ?>" data-child-interes_amount="<?php echo $_recored->interes_amount; ?>" data-child-capital="<?php echo $_recored->capital; ?>" data-child-interes="<?php echo $_recored->interes; ?>" data-child-unpaid_payments="<?php echo $_recored->unpaid_payments; ?>" data-child-due_date="<?php echo date("d-m-Y", strtotime($_recored->due_date)); ?>" >
                      ...
                      <td><?php echo number_format($_recored->unpaid_payments, 2); ?></td>
                      <td><?php echo number_format(floatval($loanstotal) + floatval($_recored->unpaid_payments) - floatval($_recored->capital_paid), 2); ?></td>
                      <td></td>
                    </tr>
                  <?php 
                    }
      ...
      <div class="nav-tabs-custom">
        ...
        <?php if($loans_origen != 'due') : ?>
          ...
          <?php 
                ...
                echo form_open('loansfixed/pay',$attributes) ?>
      ...
      <div class="nav-tabs-custom">
        ...
        <div class="tab-content">
          ...
          <?php 
                ...
                echo form_open('loansfixed/additional',$attributes) ?>
                <div class="modal-body">
...
<div class="modal modal-success fade in" tabindex="-1" role="dialog" id="completeLoanModal">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      echo form_open('loansfixed/edit/completeLoan',$attributes) ?>
        <div class="modal-body">
...
<div class="modal modal-danger fade in" tabindex="-1" role="dialog" id="removeTransactionModal">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      echo form_open('loansfixed/delete/removeTransaction',$attributes) ?>
        <div class="modal-body">
...
<div class="modal modal-danger fade in" tabindex="-1" role="dialog" id="removeAdditionalModal">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      echo form_open('loansfixed/delete/removeAdditional',$attributes) ?>
        <div class="modal-body">
...
<script type="text/javascript">
  ...
  var total_totalloans = 0;

/* Formatting function for row details - modify as you need */
function format(value, name, loans_amount, current_balance, interes_amount, capital, interes, unpaid_payments, due_date) {
  base_url = '<?php echo base_url() ?>loansfixed/';
  ...
  $(document).ready(function(){
      ...
      language: {
        ...
        /*columnDefs: [
            {
                ...
                render: function (data, type, row) {
                  total_totalloans += (removeCommas(row[2]) + removeCommas(row[3]));
                  return data + addCommas(removeCommas(row[2]) + removeCommas(row[3]));
                },
                targets: 4,
            },
        ],*/
        footerCallback: function (row, data, start, end, display) {
            ...
            // Update footer
            $(api.column(2).footer()).html(addCommas(total_loans));
            $(api.column(3).footer()).html(addCommas(total_additional));
            $(api.column(4).footer()).html(addCommas(total_totalloans));
            $(api.column(12).footer()).html(addCommas(totals));
        },        
       columns: [
            { data: 'name' },
            { data: 'date' },
    ...
    $('#example tbody').on('click', 'td', function () {
        ...
        if (row.child.isShown()) {
            ...
            // Open this row
            row.child( format(tr.data('child-value'), tr.data('child-name'), tr.data('child-loans_amount'), tr.data('child-current_balance'), tr.data('child-interes_amount'), tr.data('child-capital'), tr.data('child-interes'), tr.data('child-unpaid_payments'), tr.data('child-due_date')) ).show();
            tr.addClass('shown');
        }        
        ...
        //if (typeof tr.data('child-value') !== "undefined") { document.querySelector('ul.nav.nav-tabs li a').setAttribute("onclick", "updateOrderTransactionsTable(" + tr.data('child-value') + ", '" + due + "')"); }
  ...
  {
    url = '<?php echo base_url() ?>loansfixed/pdf/loan_letter/'+_loanno;
  ...
  {
    url = '<?php echo base_url() ?>loansfixed/pdf/loan_1/'+_loanno;
  ...
  {
    url = '<?php echo base_url() ?>loansfixed/pdf/loan_contract/'+_loanno;
  ...
  {
    url = '<?php echo base_url() ?>loansfixed/pdf/loan_receipt/'+_loanno+'/'+_transno;
  ...
  function updateOrderTransactionsTable(id, due = "")
  {
    if(id) {
      $.ajax({
          type: "GET",
          url: "<?php echo base_url() ?>ajax/getloansfixedtransactions/index",
  ...
  function updateAdditionalTable(id)
  {
    if(id) {
      $.ajax({
          type: "GET",
          url: "<?php echo base_url() ?>ajax/getloansfixedadditional/index",
          data: {"<?php echo $this->security->get_csrf_token_name(); ?>":"<?php echo $this->security->get_csrf_hash(); ?>",'id':id}
      }).done(function( msg ) { 
  ...
  function addCommas(x) { const formatterDolar = new Intl.NumberFormat('en-EN', { maximumSignificantDigits: 2 });
