...
?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>Préstamos rápidos <?php echo empty($loansdue) ? 'activos' : $loansdue ; ?> (<?php echo count($recored); ?>)</h1>
      <div class="breadcrumb">
        <?php if($loans_origen == 'due') : ?>
        <a id="loans" href="<?php echo base_url().'loansquickbusiness'; ?>" class="btn btn-lg btn-success">Préstamos activos</a>
        <?php else : ?>
        <a id="loans_due" href="<?php echo base_url().'loansquickbusiness/due'; ?>" class="btn btn-lg btn-danger">Préstamos saldados</a>
				<?php endif; ?>
        <!-- <a id="receipts" href="<?php echo base_url().'loansquickbusiness/receipt'; ?>" class="btn btn-lg btn-success">Recibos</a> -->
        <a id="new_loan" href="<?php echo base_url().'loansquickbusiness/create'; ?>" class="btn btn-lg btn-primary">Nuevo Préstamo</a> 
      </div>
    </section>
        ...
        <div class="col-md-12">
            <div class="box box-primary">
              ...
              <?php 
                ...
                echo form_open('loansquickbusiness/delete',$attributes) ?>
                <table id="example" class="display table cell-border row-border hover" style="width:100%">
                  ...
                  <tr data-child-value="<?php echo $_recored->loans_no; ?>" data-child-name="<?php echo  $_recored->customer_first_name; ?>" data-child-loans_amount="<?php echo $_recored->loans_amount; ?>"  data-child-current_balance="<?php echo $loanstotal_capital; ?>" data-child-interes_amount="<?php echo $_recored->interes_amount; ?>" data-child-capital="<?php echo $_recored->capital; ?>" data-child-interes="<?php echo $_recored->interes; ?>" data-child-unpaid_payments="<?php echo (floatval($_recored->unpaid_payments) + floatval($_recored->total_unpay)); ?>" data-child-due_date="<?php echo date("d-m-Y", strtotime($_recored->due_date)); ?>" >
                      ...
                      <td><?php echo number_format($_recored->aditional_amount, 2); ?></td>
                      ...
                      <td><?php echo number_format($_recored->capital_paid, 2); ?></td>
                      <td><?php echo number_format(floatval($_recored->unpaid_payments) + floatval($_recored->total_unpay), 2); ?></td>
                      <td><?php echo number_format(floatval($loanstotal) + floatval($_recored->interes_amount) + floatval($_recored->unpaid_payments) + floatval($_recored->total_unpay) - floatval($_recored->capital_paid), 2); ?></td>
                      <td></td>
                    </tr>
                  <?php 
                    }
      ...
      <div class="nav-tabs-custom">
        ...
        <div class="tab-content">
          ...
          <?php 
                ...
                echo form_open('loansquickbusiness/pay',$attributes) ?>
                <div class="modal-body">
                  ...
                  <div class="form-group">
                  <label for="unpaid_amount" class="col-sm-3 control-label">CAPITAL</label>
                    <div class="col-sm-3">
                  ...
                  <div class="form-group">
                    ...
                    <div class="col-sm-3">
                      <input type="text" class="form-control" id="unpaid_interes" name="unpaid_interes" >
                    </div>
      ...
      <div class="nav-tabs-custom">
        ...
        <div class="tab-content">
          ...
          <?php 
                ...
                echo form_open('loansquickbusiness/additional',$attributes) ?>
                <div class="modal-body">
                  ...
                  <div class="form-group">
                    <label for="additional_amount" class="col-sm-3 control-label">MONTO</label>
                    <div class="col-sm-8">
...
<div class="modal modal-success fade in" tabindex="-1" role="dialog" id="completeLoanModal">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      echo form_open('loansquickbusiness/edit/completeLoan',$attributes) ?>
        <div class="modal-body">
          <p>¿Seguro que quiere saldar este préstamo?</p>
        </div>
        <div class="modal-footer">
          <input type="hidden" name="txt_loanscompleteno" id="txt_loanscompleteno" value="" />
          <button type="button" class="btn btn-danger" data-dismiss="modal">Cerrar</button>
...
<div class="modal modal-danger fade in" tabindex="-1" role="dialog" id="removeTransactionModal">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      echo form_open('loansquickbusiness/delete/removeTransaction',$attributes) ?>
        <div class="modal-body">
...
<div class="modal modal-danger fade in" tabindex="-1" role="dialog" id="removeAdditionalModal">
  <div class="modal-dialog" role="document">
    <div class="modal-content">
      ...
      echo form_open('loansquickbusiness/delete/removeAdditional',$attributes) ?>
        <div class="modal-body">
...
function format(value, name, loans_amount, current_balance, interes_amount, capital, interes, due_date) {
  base_url = '<?php echo base_url() ?>loansquickbusiness/';
  ...
  $(document).ready(function(){
      ...
      language: {
        ...
        columns: [
          { data: 'name' },
            ...
            { data: 'aditional_amount', 'className': 'danger' },
    ...
    $('#example tbody').on('click', 'td', function () {
        ...
        if (row.child.isShown()) {
            ...
            // Open this row
            row.child( format(tr.data('child-value'), tr.data('child-name'), tr.data('child-loans_amount'), tr.data('child-current_balance'), tr.data('child-interes_amount'), tr.data('child-capital'), tr.data('child-interes'), tr.data('child-due_date')) ).show();
            tr.addClass('shown');
        }        
        
        if( typeof tr.data('child-value') !== "undefined")
        { document.querySelector('ul.nav.nav-tabs li a').setAttribute("onclick","updateOrderTransactionsTable("+tr.data('child-value')+")"); }
  ...
  {
    url = '<?php echo base_url() ?>loansquickbusiness/pdf/loan_letter/'+_loanno;
  ...
  {
    url = '<?php echo base_url() ?>loansquickbusiness/pdf/loan_1/'+_loanno;
  ...
  {
    url = '<?php echo base_url() ?>loansquickbusiness/pdf/loan_contract/'+_loanno;
    
    var w = 900;
    var h = 600;
    var left = (screen.width/2)-(w/2);
    var top = (screen.height/2)-(h/2);
    window.open(url,"__blank","resizable=yes,location=no,menubar=no,scrollbars=yes,status=no,toolbar=no,fullscreen=no,dependent=no,copyhistory=no,width="+w+",height="+h+",left="+left+",top="+top);
  }
  function print_receipt(_loanno, _transno)
  {
    url = '<?php echo base_url() ?>loansquickbusiness/pdf/loan_receipt/'+_loanno+'/'+_transno;
  ...
  function updateOrderTransactionsTable(id)
  {
    if(id) {
      $.ajax({
          type: "GET",
          url: "<?php echo base_url() ?>ajax/getloansquickbusinesstransactions/index",
          data: {"<?php echo $this->security->get_csrf_token_name(); ?>":"<?php echo $this->security->get_csrf_hash(); ?>",'id':id}
      }).done(function( msg ) { 
  ...
  function updateAdditionalTable(id)
  {
    if(id) {
      $.ajax({
          type: "GET",
          url: "<?php echo base_url() ?>ajax/getloansquickbusinessadditional/index",
          data: {"<?php echo $this->security->get_csrf_token_name(); ?>":"<?php echo $this->security->get_csrf_hash(); ?>",'id':id}
      }).done(function( msg ) { 
