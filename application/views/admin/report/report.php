...
?>
  ...
  <div class="content-wrapper">
    ...
    <section class="content-header">
      <h1>Reporte General <?php //var_dump( $order); ?></h1>
      <div class="breadcrumb">
        <a target="_blank" href="<?php echo base_url().'report/pdf'.'/all/'.$dateform.'/'.$dateto; ?>" class="btn btn-danger btn-lg" style="margin-right: 5px;">Exportar a PDF</a>
        <!-- <a target="_blank" href="<?php echo base_url().'report/excel'; ?>" class="btn btn-success btn-lg" style="margin-right: 5px;">Exportar a Excel</a> -->
      </div>
    </section>
                    ...
                    <table id="example1" class="display" style="width:100%">
                      ...
                      <tbody>
                        <?php 
                          ...
                          foreach ($order as $_date) {
                            if(($_date->loanscapital_amount + $_date->loanschristmas_amount + $_date->loansfixed_amount + $_date->loansinversion_amount + $_date->loansquickbusiness_amount) > 0)
                            {
                            ?>
                            <tr>
                              <td><?php echo $_date->customer_first_name; ?></td>
                              <td><?php echo $_date->customer_personalid; ?></td>
                              <td><?php echo number_format($_date->loanscapital_amount,2); ?></td>
                              <td><?php echo number_format($_date->loanschristmas_amount,2); ?></td>
                              <td><?php echo number_format($_date->loansfixed_amount,2); ?></td>
                              <td><?php echo number_format($_date->loansinversion_amount,2); ?></td>
                              <td><?php echo number_format($_date->loansquickbusiness_amount,2); ?></td>
                              <td></td>
                            </tr>
                            <?php
                          }
                      ...
                      <tfoot>
                        <tr class="bg-blue">
                          ...
                          <th width="10%">Inversión</th>
                          <th width="10%">Rápidos</th>
                          <th width="15%" id="totals">Total</th>
                        </tr>
                        </tfoot>
                    </table>
                </div>
    ...
    $('#example1').DataTable({
      language: {
        ...
        columnDefs: [
            {
                ...
                render: function (data, type, row) {
                  totals += removeCommas(row[2]) + removeCommas(row[3]) + removeCommas(row[4]) + removeCommas(row[5]) + removeCommas(row[6]);
                  return data + addCommas(removeCommas(row[2]) + removeCommas(row[3]) + removeCommas(row[4]) + removeCommas(row[5]) + removeCommas(row[6]));
                },
                targets: 7,
            },
        ],
        footerCallback: function (row, data, start, end, display) {
            ...
            // Update footer
            $(api.column(2).footer()).html(addCommas(total_capital));
            $(api.column(3).footer()).html(addCommas(total_christmas));
            $(api.column(4).footer()).html(addCommas(total_fixed));
            $(api.column(5).footer()).html(addCommas(total_inversion));
            $(api.column(6).footer()).html(addCommas(total_quickbusiness));
            $(api.column(7).footer()).html(addCommas(totals/3));
        },
    });
  });
  // get loanstype
