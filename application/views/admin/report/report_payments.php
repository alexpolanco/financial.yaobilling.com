...
<!-- Content Wrapper. Contains page content -->
  ...
  <?php 
    ...
    echo form_open('report/report_payments',$attributes) ?>

    <!-- Content Header (Page header) -->
    <section class="content-header">
      <h1>Reporte de pagos</h1>
      <div class="breadcrumb">
        <a target="_blank" href="<?php echo base_url().'report/pdf'.'/'.$loanstype.'/'.$dateform.'/'.$dateto.'/payments'; ?>" class="btn btn-danger btn-lg pull-right" style="margin-right: 5px;">Exportar a PDF</a>
        <!-- <a target="_blank" href="<?php echo base_url().'report/excel'.'/'.$loanstype.'/'.$dateform.'/'.$dateto.'/payments'; ?>" class="btn btn-success btn-lg btn-lg" style="margin-right: 5px;">Exportar a Excel</a> -->
      </div>
    </section>
    ...
    $('#example1').DataTable({
      language: {
        ...
        footerCallback: function (row, data, start, end, display) {
            ...
            // Update footer
            $(api.column(2).footer()).html(addCommas(total_cuote));
            $(api.column(3).footer()).html(addCommas(total_payed));
            $(api.column(4).footer()).html(addCommas(total_left));
        },
    });
  });
  // get loanstype
