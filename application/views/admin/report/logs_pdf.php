...
$pdf->SetSubject('PDF');
$pdf->SetKeywords('TCPDF, PDF, Reporte de Eventos, Eventos');
...
$output .= '
	<table width="100%" class="display">
		...
		<tr><td colspan="7"><h2>Reporte de Eventos '.(!empty($recored[0]->datefrom) ? ' desde ' . date('d/m/Y', strtotime($recored[0]->datefrom)) . ' hasta ' . date('d/m/Y', strtotime($recored[0]->dateto)) : '').'</h2><br></td></tr>
		<tr style="background-color:#dedede;color:#000;font-size:12px;font-weight:900;">
			<th width="10%" style="border:1px solid #000;">Fecha</th>
			<th width="20%" style="border:1px solid #000;">Operación</th>
		...
		foreach ($recored as $_date) {
			...
			$output .='<tr>
				...
				<td style="border:1px solid #000;">'.$_date->Operation.'</td>
...
$pdf->Output('Reporte_de_Eventos.pdf', 'I');
