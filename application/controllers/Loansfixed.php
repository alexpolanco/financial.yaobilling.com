...
class Loansfixed extends MY_Controller  { 
...
public function __construct()
	...
	{
		$data['recored'] = $this->loansfixed_model->findAll();
		$this->load->view('admin/loansfixed/loans-list',$data);
	}
	...
	*/
	public function pay()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			if (floatval($this->input->post('paid_amount')) > 0 || $this->input->post('paid_amount') != "0.00") {
				$next_transaction_id = $this->loansfixed_model->getLastOrderTransactionId();
				$interes_amount = $this->loansfixed_model->getOrdersData($id);
				...
				$transaction_data = array(
					...
					'interes_paid' => str_replace(",","",$this->input->post('paid_amount')) > $interes_amount['interes_amount'] ? $interes_amount['interes_amount'] : str_replace(",","",$this->input->post('paid_amount')),
					'capital_paid' => str_replace(",","",$this->input->post('paid_amount')) > $interes_amount['interes_amount'] ? str_replace(",","",$this->input->post('paid_amount')) - $interes_amount['interes_amount'] : $interes_amount['capital'],
				...
				$this->logger->write( "Fijo", "agregado", 'Préstamo: '.$id.' Pago de cliente: ' . $this->input->post('payer_name') . ' Balance anterior $' . $balance_before_update . ' Monto pagado $' . $this->input->post('paid_amount') . ' ' . ($interes_amount['interes_amount'] == str_replace(",","",$this->input->post('paid_amount')) ? "INTERES" : "PAGO IRREGULAR") . ' $' . $balance_before_update . ' BALANCE ACTUAL'  );

				$create = $this->loansfixed_model->pay($transaction_data, $accounting_data, $due_date, $id);	
			}
			...
			if (floatval($this->input->post('paid_payments_amount')) > 0) {
				...
				$this->logger->write( "Fijo", "agregado", 'Préstamo: '.$id.' Abono pago irregular - cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('paid_payments_amount') . ' PAGO IRREGULAR'  . ' $' . $balance_before_update . ' BALANCE ACTUAL' );

				$createIrregular = $this->loansfixed_model->pay($transaction_data, $accounting_data, $due_date, $id);
			}

			if (floatval($this->input->post('capitalpaid_amount')) > 0 || !empty($this->input->post('capitalpaid_amount'))) {
				$next_transaction_id = $this->loansfixed_model->getLastOrderTransactionId();
				$orders = $this->loansfixed_model->getOrdersData($id);
				...
				$capitalpaid_amount = floatval($orders["due_amount"]) - str_replace(",","",$this->input->post('capitalpaid_amount'));
				...
				$this->logger->write( "Fijo", "agregado", 'Préstamo: '.$id.' Abono al capital de cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('capitalpaid_amount') . ' ' );

				$createCapital = $this->loansfixed_model->payCapital($capital_data, $order_data, $id);
			}
			...
			if($create == true || $createIrregular == true || $createCapital == true) {
				...
				if($createCapital == true) {
					...
					$checkEndLoan = $this->loansfixed_model->CheckEndLoan($id);
	...
	public function additional()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$this->logger->write( "Fijo", "agregado", 'Préstamo: '.$id.' Adicional al capital, cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('additional_amount') );

			$create = $this->loansfixed_model->additional($transaction_data, $order_data, $id);
	...
	public function due()
	{
		...
		$data['recored'] = $this->loansfixed_model->findAllDue();
		$this->load->view('admin/loansfixed/loans-list',$data);
	}
	...
	public function receipt()
	{
		...
		$this->db->select("printed, con.due_date, loansfixed.loans_no, con.payment_no, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, FORMAT(loansfixed.loans_amount,2) loans_amount, FORMAT(loansfixed.cuota,2) cuota, lt.loanstype_name, loansfixed.entry_date, loansfixed.start_date, loansfixed.end_date, FORMAT(loansfixed.due_amount,2) due_amount, loansfixed.interes interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, a.routes_name,a.collector_id, IF(cob.collector_nickname='', cob.collector_first_name, CONCAT_WS(' ', cob.collector_first_name, '(', cob.collector_nickname, ')')) collector_first_name, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loansfixed_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loansfixed.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount");
		
		$this->db->join('customer cli', 'loansfixed.customer_id=cli.customer_id', 'left');
		$this->db->join('customer cli2', 'loansfixed.customer_id=cli2.customer_id', 'left');
		$this->db->join('routes a', 'loansfixed.routes_id=a.routes_id', 'left');
		$this->db->join('collector cob', 'a.collector_id=cob.collector_id', 'left');
		$this->db->join('loanstype lt', 'loansfixed.loanstype_id=lt.loanstype_id', 'left');
		$this->db->join('loansfixed_accounting con', 'con.loans_no = loansfixed.loans_no');
		...
		if(isset($customer_field) && $customer_field != ""){
			$this->db->where('loansfixed.customer_id', $customer_field);
		}
		if(isset($routes_field) && $routes_field != ""){
			$this->db->where('loansfixed.routes_id', $routes_field);
		}
		...
		$data['recored'] = $this->db->get('loansfixed')->result();
		$this->load->view('admin/loansfixed/loans-receipt',$data);
	}
	...
	public function receipt_due()
	{
		...
		$this->db->select("printed, con.due_date, loansfixed.loans_no, (lt.loanstype_duration + 1) payment_no, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, FORMAT(loansfixed.loans_amount,2) loans_amount, FORMAT(loansfixed.cuota,2) cuota, lt.loanstype_name, loansfixed.entry_date, loansfixed.start_date, loansfixed.end_date, FORMAT(loansfixed.due_amount,2) due_amount, loansfixed.interes interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, a.routes_name,a.collector_id, IF(cob.collector_nickname='', cob.collector_first_name, CONCAT_WS(' ', cob.collector_first_name, '(', cob.collector_nickname, ')')) collector_first_name, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loansfixed_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loansfixed.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount, '500.00' fee_payment");  
	...
	public function return_loans()
	{
		...
		$data['recored'] = $this->db->get('loansfixed')->result();
		$this->load->view('admin/loansfixed/loans-return-list',$data);
	}

	// order view
	public function loan_view()
	{
		$this->load->view('admin/loansfixed/loan-view');
	}
	...
	public function pdf($segment_1,$segment_2,$segment_3="",$segment_4="")
	{
		if($segment_1 == "loan_1" || $segment_1 == "loan_letter" || $segment_1 == "loan_contract" || $segment_1 == "loan_receipt")
		{
			...
			else if($segment_1 == "loan_letter")
			{
				...
				loansfixed.duration, loansfixed.interes_amount, FORMAT(loansfixed.loans_amount,2) loans_amount, FORMAT(loansfixed.cuota,2) cuota, lt.loanstype_name, lt.loanstype_frequency, loansfixed.contract_number, loansfixed.contract_folio1, loansfixed.contract_folio2, loansfixed.entry_date, loansfixed.entry_time, loansfixed.start_date, loansfixed.end_date, FORMAT(loansfixed.due_amount,2) due_amount, loansfixed.interes interes, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loansfixed_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loansfixed.loans_no) atraso, cli2.customer_address garante_address, cli2.customer_phone garante_phone ");
			
				$this->db->join('customer cli', 'loansfixed.customer_id=cli.customer_id', 'left');
				$this->db->join('customer cli2', 'loansfixed.customer_id=cli2.customer_id', 'left');
				$this->db->join('guarantor g1', 'loansfixed.guarantor1_id=g1.guarantor_id', 'left');
				$this->db->join('guarantor g2', 'loansfixed.guarantor2_id=g2.guarantor_id', 'left');
				//$this->db->join('routes a', 'loansfixed.routes_id=a.routes_id', 'left');
				//$this->db->join('collector cob', 'a.collector_id=cob.collector_id', 'left');
				$this->db->join('loanstype lt', 'loansfixed.loanstype_id=lt.loanstype_id', 'left');
				...
				$this->db->where('loansfixed.company_id', $this->session->userdata('company_id'));			
				$this->db->where('loansfixed.loans_no =', $segment_2);
						
				$this->db->order_by('customer_first_name', 'ASC');
				
				$data['order'] = $this->db->get('loansfixed')->result();
				$data['date1'] = $segment_2;
				$data['date2'] = $segment_3;
				$this->load->view('admin/loansfixed/'.$segment_1,$data);
			}
			...
			{
				$this->db->select("printed, con.due_date, loansfixed.loans_no, loansfixed.current_balance, tra.payment_date, tra.transactions_type, tra.amount transactions_amount,  con.payment_no, cli.customer_gender, cli.customer_nationality, cli.customer_civilstatus, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, loansfixed.duration, loansfixed.interes_amount, FORMAT(loansfixed.loans_amount,2) loans_amount, FORMAT(loansfixed.cuota,2) cuota, lt.loanstype_name, lt.loanstype_frequency, loansfixed.contract_number, loansfixed.contract_folio1, loansfixed.contract_folio2, loansfixed.entry_date, loansfixed.entry_time, loansfixed.start_date, loansfixed.end_date, FORMAT(loansfixed.due_amount,2) due_amount, loansfixed.interes interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loansfixed_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loansfixed.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount");
				...
				$this->db->join('loanstype lt', 'loansfixed.loanstype_id=lt.loanstype_id', 'left');
				$this->db->join('loansfixed_accounting con', 'con.loans_no = loansfixed.loans_no');
				$this->db->join('loansfixed_transactions tra', 'tra.loans_no = loansfixed.loans_no', 'left outer');
		...
		else
		{
			...
			$this->db->select("printed, con.due_date, loansfixed.loans_no, con.payment_no, cli.customer_gender, cli.customer_nationality, cli.customer_civilstatus, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, FORMAT(loansfixed.loans_amount,2) loans_amount, FORMAT(loansfixed.cuota,2) cuota, lt.loanstype_name, loansfixed.entry_date, loansfixed.start_date, loansfixed.end_date, FORMAT(loansfixed.due_amount,2) due_amount, loansfixed.interes interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, a.routes_name,a.collector_id, IF(cob.collector_nickname='', cob.collector_first_name, CONCAT_WS(' ', cob.collector_first_name, '(', cob.collector_nickname, ')')) collector_first_name, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT IFNULL(SUM(ct.cuota - ct.advance_amount), 0) FROM loansfixed_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loansfixed.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount");
	...
	{
		$data['recored'] = $this->loansfixed_model->findAll();
		$this->load->view('admin/loansfixed/loans-excel',$data);
	}
	...
	public function get_transactions_data($no)
	{
		...
		$query1 = $this->db->get('loansfixed_accounting')->result();

		$this->db->where('company_id', $this->session->userdata('company_id'));
		$this->db->where('loans_no',$no);
		$query2 = $this->db->get('loansfixed_transactions')->result();
	...
	public function get_loan_number($no)
	{
		$this->db->where('company_id', $this->session->userdata('company_id'));
		$this->db->where('loans_no',$no);
		$data = $this->db->get('loansfixed')->row_array();
	...
	public function create()
	{
		...
		{
			$this->load->view('admin/loansfixed/loans-add');
		}
		else
		{
			$this->loansfixed_model->insert();
	...
	{
		$this->db->join('customer', 'loansfixed.customer_id=customer.customer_id', 'left');
		$this->db->join('routes', 'loansfixed.routes_id=routes.routes_id', 'left');
		$this->db->join('collector', 'routes.collector_id=collector.collector_id', 'left');
		$this->db->join('loanstype', 'loansfixed.loanstype_id=loanstype.loanstype_id', 'left');
		$this->db->join('loansfixed_accounting la', 'loansfixed.loans_no=la.loans_no', 'left');

		$this->db->where('customer.company_id', $this->session->userdata('company_id'));
		$this->db->where('loansfixed.company_id', $this->session->userdata('company_id'));
		
		//$this->db->where('loansfixed.loans_no',$no);

		$data['loansfixed'] = $this->db->get('loansfixed')->result();
		
		//$this->load->view('admin/loansfixed/loan-pos-confirm',$data);
		//$this->load->view('admin/loansfixed/loan-print-confirm',$data);
		$this->load->view('admin/loansfixed/loan-report-confirm',$data);
	}
	...
	{		
		...
		{
			$this->loansfixed_model->update($id);
			return redirect('loansfixed');
		}
		else
		{
			...
			$this->db->update('loansfixed_transactions',$data);

			$this->logger->write( "Fijo", "edicion", 'Transaccion: ' . $id . ' nuevo valor interes: ' . $_POST['edit_new_interes_amount'] . ' nuevo valor capital: ' . $_POST['edit_new_capital_amount'] );
		...
		/*if(isset($_POST['return']))
		{
			if($_POST['return'] == 'yes')
			{
				...
				$this->db->update('loansfixed',$data);
		...
		else
		{
			$this->db->where('company_id', $this->session->userdata('company_id'));
			$this->db->where('loans_no',$id);
			$this->db->delete('loansfixed');

			//$this->db->where('grand_order_no',$id);
			//$this->db->delete('loans_grandtotal');

			$data_ar = $this->loansfixed_model->insert();
			//echo $data_ar;
			//return redirect('loansfixed/loan_print/'.$data_ar);
			return redirect('loansfixed');
		}*/
	}
	...
	{		
		$data['recored'] = $this->loansfixed_model->findOne($id);
		$this->load->view('admin/loansfixed/loans-edit',$data);
	}
	...
	{		
		$data['recored'] = $this->loansfixed_model->findOne($id);
		$this->load->view('admin/loansfixed/loans-renew',$data);
	}
	...
	public function delete($id)
	{
		...
		{
			$rights = $this->check_rights();
			if(!in_array('loansfixed/delete', $rights))
			{
			...
			else
			{
				if($id == "removeTransaction"){
					...
					$orders = $this->loansfixed_model->getOrdersData($this->input->post('loan_no'));
					$transaction = $this->loansfixed_model->getTransactionByID($this->input->post('transactions_no'));
					...
					$this->logger->write( "Fijo", "eliminado", 'Préstamo: '.$order_id.' Abono del cliente: ' . $orders['customer_first_name'] . ' monto ' . $transaction['transactions_type'] . ' $' . $transaction_amount );

					$this->loansfixed_model->removeTransaction($this->input->post('loan_no'), $this->input->post('transactions_no'), $order_data);
					...
					$this->logger->write( "Fijo", "eliminado", 'Préstamo: '.$order_id.' Adicional del cliente: ' . $orders['customer_first_name'] . ' monto ' . $transaction['transactions_type'] . ' $' . $transaction_amount );

					$this->loansfixed_model->removeAdditional($this->input->post('loan_no'), $this->input->post('transactions_no'), $order_data);					
					$this->session->set_flashdata('msg','Datos eliminados exitosamente');	
				}
				else{
					$orders = $this->loansfixed_model->getOrdersData($id);
					...
					$this->logger->write( "Fijo", "eliminado", 'Préstamo: '.$orders['loans_no'].' Cliente: ' . $orders['customer_first_name'] . ' monto $' . $orders['current_balance'] . ' duración: ' . $orders['duration'] . ' ' . $_frequency . ' interes: ' . $orders['interes_amount']);
					
					$this->loansfixed_model->remove($id);
					$this->session->set_flashdata('msg','Datos eliminados exitosamente');
				}
	...
	{
		$this->loansfixed_model->change_status($id,$mode);
		return redirect('loansfixed');
	}
	...
	*/
	...
	{
		if($id) {
			$transaction = $this->loansfixed_model->getTransactionByID($id);
			$order_data = $this->loansfixed_model->getOrdersData($order_id);
			$loans_details = $this->loansfixed_model->getLoansDetails($order_id, $order_data['customer_id']);
			...
			<table width="100%" class="display" style="font-size:10px">
				<tr>
					<td width="68%">
						<div><span class="text-gray-light">CLIENTE PRESTAMO INTERES FIJO:</span><br>
						...
						<div style="min-width: 600px">
								...
								<main>
										...
										<table border="0" cellspacing="0" cellpadding="0">
											...
											$total_capital_paid = $this->loansfixed_model->countTotalPaidTransaction($order_id, $transaction['transactions_type']);
								...
								<a href="'.base_url().'loansfixed/" class="text-center btn btn-outline-primary hidden-print noPrint">Regresar</a>
							</div>
						</div>
					</div>
				</div>
							...
							<div class="text-center">
								...
								<div class="row contacts">
									<div class="col invoice-to">
										<div class="text-gray-light">CLIENTE PRESTAMO INTERES FIJO:</div>
										<h2 class="to">'.$order_data['customer_first_name'].'</h2>
										<div class="address">'.$order_data['customer_address'].'</div>
										<div class="email">'.$order_data['customer_phone'].'</div>
									</div>
							...
							<table class="table table-hover">
								...
								<tbody>
									...
									<tr>
										...
										<p><strong>'; $total_paid = $this->loansfixed_model->countTotalPaidOrdersTransactionByDate($order_id); $html .= sprintf("%1\$.2F", $total_paid['amount']).'</strong></p></td>
									</tr>
									<tr>
	...
	*/
	...
	{
		if($id) {
			$order_data = $this->loansfixed_model->getOrdersData($id);
			$company_info = $this->company_model->findOne($this->session->userdata('company_id'));
