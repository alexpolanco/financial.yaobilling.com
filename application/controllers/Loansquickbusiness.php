...
class Loansquickbusiness extends MY_Controller  { 
	...
	{
		$data['recored'] = $this->loansquickbusiness_model->findAll();
		$this->load->view('admin/loansquickbusiness/loans-list',$data);
	}
	...
	public function pay()
	{
		...
		$this->form_validation->set_rules('payer_date', 'Fecha del pago', 'trim|required');
		$this->form_validation->set_rules('payer_name', 'Pagado por', 'trim');
		$this->form_validation->set_rules('paid_amount', 'Cantidad a pagar', 'trim|required|numeric');
		//$this->form_validation->set_error_delimiters('<p class="text-danger">','</p>');

		$id = $this->input->post('txt_payloansno');
		$payer_date = $this->input->post('payer_date');
		$due_date = $this->input->post('txt_due_date');

		if ($this->form_validation->run() == TRUE) {
			if (floatval($this->input->post('paid_amount')) > 0 || $this->input->post('paid_amount') != "0.00") {
				$next_transaction_id = $this->loansquickbusiness_model->getLastOrderTransactionId();
				$interes_amount = $this->loansquickbusiness_model->getOrdersData($id);
				$balance_before_update = $interes_amount['current_balance'];

				$transaction_data = array(
					'transactions_no' => $next_transaction_id['transactions_no']+1,
					'loans_no' => $id,
					'payer_name' => $this->input->post('payer_name'),
					'payment_date' => $payer_date,
					'transactions_type' => $interes_amount['interes_amount'] == str_replace(",","",$this->input->post('paid_amount')) ? "INTERES" : "PAGO IRREGULAR",
					'amount' => str_replace(",","",$this->input->post('paid_amount')),
					'balance' => $balance_before_update,
					'balance_before_update' => $balance_before_update,
					'balance_before_update' => str_replace(",","",$this->input->post('unpaid_amount')),
					'interes_paid' => str_replace(",","",$this->input->post('paid_amount')) > $interes_amount['interes'] ? $interes_amount['interes'] : str_replace(",","",$this->input->post('paid_amount')),
					'capital_paid' => str_replace(",","",$this->input->post('paid_amount')) > $interes_amount['interes'] ? str_replace(",","",$this->input->post('paid_amount')) - $interes_amount['interes'] : $interes_amount['capital'],
					'concept' => $interes_amount['interes_amount'] == str_replace(",","",$this->input->post('paid_amount')) ? "PAGO DE INTERES" : "PAGO IRREGULAR",
					'accounting_due_date' => $due_date,
				...
				$this->logger->write( "Rapido", "agregado", 'Préstamo: '.$id.' Pago de cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('paid_amount') . ' ' . $interes_amount['interes_amount'] == str_replace(",","",$this->input->post('paid_amount')) ? "INTERES" : "PAGO IRREGULAR" );

				$create = $this->loansquickbusiness_model->pay($transaction_data, $accounting_data, $due_date, $id);	
			}
	
			if (floatval($this->input->post('paid_payments_amount')) > 0) {
				$next_transaction_id = $this->loansquickbusiness_model->getLastOrderTransactionId();
				$interes_amount = $this->loansquickbusiness_model->getOrdersData($id);
				$balance_before_update = $interes_amount['current_balance'];
				...
				$transaction_data = array(
					...
					'payer_name' => $this->input->post('payer_name'),
					'payment_date' => $payer_date,
					'transactions_type' => "PAGO IRREGULAR",
					'amount' => str_replace(",","",$this->input->post('paid_payments_amount')),
					'balance' => $balance_before_update,
					'balance_before_update' => $balance_before_update,
					'interes_paid' => str_replace(",","",$this->input->post('paid_payments_amount')),
					'capital_paid' => "0.00",
					'concept' => "ABONO PAGO IRREGULAR",
					'accounting_due_date' => $due_date,
					'user_id' => $this->session->userdata('userid'),
					'company_id' => $this->session->userdata('company_id')
				);
				...
				$this->logger->write( "Capital", "agregado", 'Préstamo: '.$id.' Abono pago irregular - cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('paid_payments_amount') . ' PAGO IRREGULAR' );

				$createIrregular = $this->loansquickbusiness_model->pay($transaction_data, $accounting_data, $due_date, $id);
			}

			if (floatval($this->input->post('capitalpaid_amount')) > 0 || !empty($this->input->post('capitalpaid_amount'))) {
				$next_transaction_id = $this->loansquickbusiness_model->getLastOrderTransactionId();
				$orders = $this->loansquickbusiness_model->getOrdersData($id);
				$balance = floatval($orders["current_balance"]) - str_replace(",","",$this->input->post('capitalpaid_amount'));
				$balance_before_update = $orders['current_balance'];
				...
				$capital_data = array(
					...
					'payer_name' => $this->input->post('payer_name'),
					'payment_date' => $payer_date,
					'transactions_type' => "CAPITAL",
					'amount' => str_replace(",","",$this->input->post('capitalpaid_amount')),
					'balance' => $balance,
					'balance_before_update' => $balance_before_update,
					'capital_paid' => str_replace(",","",$this->input->post('capitalpaid_amount')),
					'concept' => "ABONO AL CAPITAL",
					'accounting_due_date' => $due_date,
					'user_id' => $this->session->userdata('userid'),
					'company_id' => $this->session->userdata('company_id')
				);

				$total_paid = floatval($orders["due_amount"]) + str_replace(",","",$this->input->post('paid_amount'));
				$capitalpaid_amount = floatval($orders["due_amount"]) - str_replace(",","",floatval($this->input->post('capitalpaid_amount')));
				$cuota = floatval($balance) * floatval($orders['interes']) / 100;

				$order_data = array(
					//'paid_amount' => $total_paid,
					'due_amount' => $capitalpaid_amount,
					//'interes_amount' => $cuota,
					//'loans_amount' => str_replace(",","",$balance),
					'current_balance' => str_replace(",","",$balance)
				);

				$this->logger->write( "Rapido", "agregado", 'Préstamo: '.$id.' Abono al capital de cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('capitalpaid_amount') . ' ' );

				$createCapital = $this->loansquickbusiness_model->payCapital($capital_data, $order_data, $id);
			}
			
			if($create == true || $createIrregular == true || $createCapital == true) {
				$this->session->set_flashdata('msg','Datos insertados exitosamente');

				if($create == true || $createCapital == true) {
					$this->printTransactionDiv($id, $orders['transactions_no'], $capital_data['transactions_no']);

					$checkEndLoan = $this->loansquickbusiness_model->CheckEndLoan($id);

					if ($checkEndLoan) {
						$this->pdf('loan_letter',$id);
					}
				}
			}
			else {
		...
		return redirect('loansquickbusiness');
	}
	...
	public function additional()
	{
		...
		$this->form_validation->set_rules('additional_date', 'Fecha del adicional', 'trim|required');
		$this->form_validation->set_rules('additional_amount', 'Monto del adicional', 'trim|required|numeric');
		$this->form_validation->set_rules('new_cuota_amount', 'Nuevo valor de la cuota', 'trim|required|numeric');
		$this->form_validation->set_rules('new_interes_amount', 'Nuevo monto de abono al interes', 'trim|required|numeric');
		$this->form_validation->set_rules('new_capital_amount', 'Nuevo monto de abono al capital', 'trim|required|numeric');
		//$this->form_validation->set_error_delimiters('<p class="text-danger">','</p>');

		$id = $this->input->post('txt_loansno');
		$date = date('Y-m-d', strtotime($this->input->post('additional_date')));
		$payer_date = $this->input->post('additional_date');

		if ($this->form_validation->run() == TRUE) {
			$next_transaction_id = $this->loansquickbusiness_model->getLastOrderTransactionId();
			$orders = $this->loansquickbusiness_model->getOrdersData($id);
			$balance = floatval($orders["current_balance"]) + str_replace(",","",$this->input->post('additional_amount'));
			$balance_before_update = $orders['current_balance'];
			...
			$transaction_data = array(
				...
				'payer_name' => $this->input->post('payer_name'),
				'payment_date' => $payer_date,
				'transactions_type' => "ADICIONAL",
				'amount' => str_replace(",","",$this->input->post('additional_amount')),
				'balance' => $balance,
				'balance_before_update' => $balance_before_update,
				'concept' => "ADICIONAL AL CAPITAL",
				'accounting_due_date' => $payer_date,
				'user_id' => $this->session->userdata('userid'),
				'company_id' => $this->session->userdata('company_id')
			);

			$additional_amount = floatval($orders["additional_amount"]) + str_replace(",","",$this->input->post('additional_amount'));
			$due_amount = floatval($orders["due_amount"]) + str_replace(",","",$this->input->post('additional_amount'));
			$cuota = str_replace(",","",$this->input->post('new_cuota_amount'));
			$interes = str_replace(",","",$this->input->post('new_interes_amount'));
			$capital = str_replace(",","",$this->input->post('new_capital_amount'));
			...
			$this->logger->write( "Rapido", "agregado", 'Préstamo: '.$id.' Adicional al capital, cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('additional_amount') );

			$create = $this->loansquickbusiness_model->additional($transaction_data, $order_data, $id);

			if($create == true ) {				
				$this->session->set_flashdata('msg','Datos insertados exitosamente');
				$this->printTransactionDiv($id, $transaction_data['transactions_no']);
				return redirect('loansquickbusiness');
			}
			else {
				$this->session->set_flashdata('msg','Ocurrió un error');
	...
	public function due()
	{
		...
		$data['recored'] = $this->loansquickbusiness_model->findAllDue();
		$this->load->view('admin/loansquickbusiness/loans-list',$data);
	}
	...
	public function receipt()
	{
		...
		$this->db->join('routes a', 'loansquickbusiness.routes_id=a.routes_id', 'left');
		$this->db->join('collector cob', 'a.collector_id=cob.collector_id', 'left');
		$this->db->join('loanstype lt', 'loansquickbusiness.loanstype_id=lt.loanstype_id', 'left');
		$this->db->join('loansquickbusiness_accounting con', 'con.loans_no = loansquickbusiness.loans_no');
		...
		if(isset($customer_field) && $customer_field != ""){
			$this->db->where('loansquickbusiness.customer_id', $customer_field);
		}
		if(isset($routes_field) && $routes_field != ""){
			$this->db->where('loansquickbusiness.routes_id', $routes_field);
		}
		...
		$data['recored'] = $this->db->get('loansquickbusiness')->result();
		$this->load->view('admin/loansquickbusiness/loans-receipt',$data);
	}
	...
	public function receipt_due()
	{
		...
		$this->db->select("printed, con.due_date, loansquickbusiness.loans_no, (lt.loanstype_duration + 1) payment_no, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, FORMAT(loansquickbusiness.loans_amount,2) loans_amount, FORMAT(loansquickbusiness.cuota,2) cuota, lt.loanstype_name, loansquickbusiness.entry_date, loansquickbusiness.start_date, loansquickbusiness.end_date, FORMAT(loansquickbusiness.due_amount,2) due_amount, FORMAT(con.interes,2) interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, a.routes_name,a.collector_id, IF(cob.collector_nickname='', cob.collector_first_name, CONCAT_WS(' ', cob.collector_first_name, '(', cob.collector_nickname, ')')) collector_first_name, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loansquickbusiness_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loansquickbusiness.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount, '500.00' fee_payment");  
	...
	public function return_loans()
	{
		...
		$data['recored'] = $this->db->get('loansquickbusiness')->result();
		$this->load->view('admin/loansquickbusiness/loans-return-list',$data);
	}

	// order view
	public function loan_view()
	{
		$this->load->view('admin/loansquickbusiness/loan-view');
	}

	// pdf method
	public function pdf($segment_1,$segment_2,$segment_3="",$segment_4="")
	{
		if($segment_1 == "loan_1" || $segment_1 == "loan_letter" || $segment_1 == "loan_contract" || $segment_1 == "loan_receipt" || $segment_1 == "loan_complete")
		{
			if($segment_1 == "loan_complete")
			{
				//$this->printTransactionDiv($segment_2, $segment_3);getLastTransactionByLoan

				$data['order_data'] = $this->loansquickbusiness_model->getOrdersData($segment_2);
				$data['transaction_pay'] = $this->loansquickbusiness_model->getLastTransactionByLoan($segment_2);
				$data['transaction_capital'] = $this->loansquickbusiness_model->getTransactionByID($segment_4);
				
				$data['date1'] = $segment_2;
				$data['date2'] = $segment_3;
				$data['order'] = $this->loansquickbusiness_model->getLoanCustomer($segment_2);

				$this->load->view('admin/loansquickbusiness/loans-complete',$data);
				//$this->load->view('admin/loansquickbusiness/'.$segment_1,$data);
			}
			else if($segment_1 == "loan_receipt")
			{
				//$this->printTransactionDiv($segment_2, $segment_3);

				$data['order_data'] = $this->loansquickbusiness_model->getOrdersData($segment_2);
				$data['transaction_pay'] = $this->loansquickbusiness_model->getTransactionByID($segment_3);
				$data['transaction_capital'] = $this->loansquickbusiness_model->getTransactionByID($segment_4);
			...
			else if($segment_1 == "loan_letter")
			{
				...
				loansquickbusiness.duration, loansquickbusiness.interes_amount, FORMAT(loansquickbusiness.loans_amount,2) loans_amount, FORMAT(loansquickbusiness.cuota,2) cuota, lt.loanstype_name, lt.loanstype_frequency, loansquickbusiness.contract_number, loansquickbusiness.contract_folio1, loansquickbusiness.contract_folio2, loansquickbusiness.entry_date, loansquickbusiness.entry_time, loansquickbusiness.start_date, loansquickbusiness.end_date, FORMAT(loansquickbusiness.due_amount,2) due_amount, loansquickbusiness.interes interes, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loansquickbusiness_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loansquickbusiness.loans_no) atraso, cli2.customer_address garante_address, cli2.customer_phone garante_phone ");
				...
				$this->db->where('loansquickbusiness.company_id', $this->session->userdata('company_id'));			
				$this->db->where('loansquickbusiness.loans_no =', $segment_2);
						
				$this->db->order_by('customer_first_name', 'ASC');
				
				$data['order'] = $this->db->get('loansquickbusiness')->result();
				$data['date1'] = $segment_2;
				$data['date2'] = $segment_3;
				$this->load->view('admin/loansquickbusiness/'.$segment_1,$data);
			}
			...
			{
				$this->db->select("printed, con.due_date, loansquickbusiness.loans_no, loansquickbusiness.current_balance, tra.payment_date, tra.transactions_type, tra.amount transactions_amount,  con.payment_no, cli.customer_gender, cli.customer_nationality, cli.customer_civilstatus, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, loansquickbusiness.duration, loansquickbusiness.interes_amount, FORMAT(loansquickbusiness.loans_amount,2) loans_amount, FORMAT(loansquickbusiness.cuota,2) cuota, lt.loanstype_name, lt.loanstype_frequency, loansquickbusiness.contract_number, loansquickbusiness.contract_folio1, loansquickbusiness.contract_folio2, loansquickbusiness.entry_date, loansquickbusiness.entry_time, loansquickbusiness.start_date, loansquickbusiness.end_date, FORMAT(loansquickbusiness.due_amount,2) due_amount, loansquickbusiness.interes interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loansquickbusiness_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loansquickbusiness.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount");
				...
				$this->db->join('loanstype lt', 'loansquickbusiness.loanstype_id=lt.loanstype_id', 'left');
				$this->db->join('loansquickbusiness_accounting con', 'con.loans_no = loansquickbusiness.loans_no');
				$this->db->join('loansquickbusiness_transactions tra', 'tra.loans_no = loansquickbusiness.loans_no');
		...
		else
		{
			...
			$this->db->select("printed, con.due_date, loansquickbusiness.loans_no, con.payment_no, cli.customer_gender, cli.customer_nationality, cli.customer_civilstatus, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, FORMAT(loansquickbusiness.loans_amount,2) loans_amount, FORMAT(loansquickbusiness.cuota,2) cuota, lt.loanstype_name, loansquickbusiness.entry_date, loansquickbusiness.start_date, loansquickbusiness.end_date, FORMAT(loansquickbusiness.due_amount,2) due_amount, loansquickbusiness.interes interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, a.routes_name,a.collector_id, IF(cob.collector_nickname='', cob.collector_first_name, CONCAT_WS(' ', cob.collector_first_name, '(', cob.collector_nickname, ')')) collector_first_name, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT IFNULL(SUM(ct.cuota - ct.advance_amount), 0) FROM loansquickbusiness_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loansquickbusiness.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount");
			...
			$this->db->join('collector cob', 'a.collector_id=cob.collector_id', 'left');
			$this->db->join('loanstype lt', 'loansquickbusiness.loanstype_id=lt.loanstype_id', 'left');
			$this->db->join('loansquickbusiness_accounting con', 'con.loans_no = loansquickbusiness.loans_no', 'left outer');
	...
	{
		$data['recored'] = $this->loansquickbusiness_model->findAll();
		$this->load->view('admin/loansquickbusiness/loans-excel',$data);
	}
	...
	public function get_transactions_data($no)
	{
		...
		$query1 = $this->db->get('loansquickbusiness_accounting')->result();

		$this->db->where('company_id', $this->session->userdata('company_id'));
		$this->db->where('loans_no',$no);
		$query2 = $this->db->get('loansquickbusiness_transactions')->result();
	...
	public function get_loan_number($no)
	{
		$this->db->where('loansquickbusiness.company_id', $this->session->userdata('company_id'));
		$this->db->where('loans_no',$no);
		$data = $this->db->get('loansquickbusiness')->row_array();
	...
	public function create()
	{
		...
		{
			$this->load->view('admin/loansquickbusiness/loans-add');
		}
		else
		{
			$this->loansquickbusiness_model->insert();
	...
	public function loan_print($no = null)
	{
		...
		$this->db->join('routes', 'loansquickbusiness.routes_id=routes.routes_id', 'left');
		...
		$this->db->join('loansquickbusiness_accounting la', 'loansquickbusiness.loans_no=la.loans_no', 'left');

		$this->db->where('loansquickbusiness.company_id', $this->session->userdata('company_id'));
		//$this->db->where('loansquickbusiness.loans_no',$no);

		$data['loansquickbusiness'] = $this->db->get('loansquickbusiness')->result();
		
		//$this->load->view('admin/loansquickbusiness/loan-pos-confirm',$data);
		//$this->load->view('admin/loansquickbusiness/loan-print-confirm',$data);
		$this->load->view('admin/loansquickbusiness/loan-report-confirm',$data);
	}
	...
	public function update($id)
	{
		...
		{
			$this->loansquickbusiness_model->update($id);
			return redirect('loansquickbusiness');
		}
		else
		{
			...
			$this->db->update('loansquickbusiness_transactions',$data);

			$this->logger->write( "Rapidos", "edicion", 'Transaccion: ' . $id . ' nuevo valor interes: ' . $_POST['edit_new_interes_amount'] . ' nuevo valor capital: ' . $_POST['edit_new_capital_amount'] );
		...
		else
		{
			...
			$data_ar = $this->loansquickbusiness_model->insert();
			//echo $data_ar;	
			//return redirect('loansquickbusiness/loan_print/'.$data_ar);
			return redirect('loansquickbusiness');
		}
	}
	...
	public function edit($id)
	{
		...
		if($id == "completeLoan"){
			...
			$id = $this->input->post('txt_loanscompleteno');
			//$id = $id_array[1];
			//$loans_complete = $this->input->get('id');
			...
			$capital_data = array(
				...
				'payment_date' => $payer_date,
				'transactions_type' => "CAPITAL",
				'amount' => $balance_before_update,
				'balance' => $balance,
				'balance_before_update' => $balance_before_update,
				'interes_paid' => str_replace(",","",$orders['interes']),
				'capital_paid' => $balance_before_update,
				'concept' => "SALDO DEL CAPITAL",
				'accounting_due_date' => date("d-m-Y"),
				'user_id' => $this->session->userdata('userid'),
				'company_id' => $this->session->userdata('company_id')
			);

			//$total_paid = floatval($orders["due_amount"]) + $balance_before_update;
			$capitalpaid_amount = floatval($orders["due_amount"]) - $balance_before_update;
			//$cuota = "0.00";
			...
			$this->logger->write( "Rapido", "agregado", 'TEST: Préstamo: '.$id.' Saldo del préstamo de cliente: ' . $this->input->post('payer_name') . ' $' . $balance_before_update . ' Balance actual: '. $orders['current_balance'] . '  Debido: ' . $capitalpaid_amount);
			...
			if($create == true || $createCapital == true) {
				...
				if($create == true || $createCapital == true) {
					...
					//$checkEndLoan = $this->loansquickbusiness_model->CheckEndLoan($id);
					//if ($checkEndLoan) {
						$this->pdf('loan_complete',$id, $next_transaction_id['transactions_no']);
						//$this->pdf('loan_letter',$id);
					//}
				}
			}
			else {
				$this->session->set_flashdata('msg','Ocurrió un error'.' due_amount: ' .$capitalpaid_amount. ' balance_before_update: ' . $balance_before_update . ' current_balance: '.$balance);
			}
			...
			$data['recored'] = $this->loansquickbusiness_model->findOne($id);
			$this->load->view('admin/loansquickbusiness/loans-edit',$data);
		}
	}
	...
	{		
		$data['recored'] = $this->loansquickbusiness_model->findOne($id);
		$this->load->view('admin/loansquickbusiness/loans-renew',$data);
	}	
	...
	public function delete($id)
	{
		...
		{
			$rights = $this->check_rights();
			if(!in_array('loansquickbusiness/delete', $rights))
			{
				return redirect('access');
			}
			else
			{
				if($id == "removeTransaction"){
				
					$id = $this->input->post('transactions_no');
					$order_id = $this->input->post('loan_no');
					$transaction_amount = $this->input->post('amount');
					$transaction_interes = $this->input->post('interes');
					$due_date = $this->input->post('due_date');
					
					$orders = $this->loansquickbusiness_model->getOrdersData($this->input->post('loan_no'));
					$transaction = $this->loansquickbusiness_model->getTransactionByID($this->input->post('transactions_no'));

					if ($transaction['transactions_type'] == "CAPITAL") {
						$current_balance = $orders["current_balance"] + $transaction['amount'];
						$interes_amount = floatval($current_balance) * floatval($orders['interes']) / 100;
	
						$order_data = array(
							'due_amount' => $orders["due_amount"] + $transaction['amount'],
							//'interes_amount' => $interes_amount,
							'loans_amount' => $balance,
							'current_balance' => $current_balance
						);							
					}

					$this->logger->write( "Rapido", "eliminado", 'Préstamo: '.$order_id.' Abono del cliente: ' . $orders['customer_first_name'] . ' monto ' . $transaction['transactions_type'] . ' $' . $transaction_amount );

					$this->loansquickbusiness_model->removeTransaction($this->input->post('loan_no'), $this->input->post('transactions_no'), $order_data);
					$this->session->set_flashdata('msg','Datos eliminados exitosamente');	
				}
				else if($id == "removeAdditional"){
					...
					$order_data = array(
						'additional_amount' => $orders["additional_amount"] - $transaction['amount'],
						'due_amount' => $orders["due_amount"] - $transaction['amount'],
						//'interes_amount' => $interes_amount,
						'current_balance' => $current_balance
					);

					$this->logger->write( "Rapido", "eliminado", 'Préstamo: '.$order_id.' Adicional del cliente: ' . $orders['customer_first_name'] . ' monto ' . $transaction['transactions_type'] . ' $' . $transaction_amount );

					$this->loansquickbusiness_model->removeAdditional($this->input->post('loan_no'), $this->input->post('transactions_no'), $order_data);					
					$this->session->set_flashdata('msg','Datos eliminados exitosamente');	
				}
				else{
					$orders = $this->loansquickbusiness_model->getOrdersData($id);
					...
					$this->logger->write( "Rapido", "eliminado", 'Préstamo: '.$orders['loans_no'].' Cliente: ' . $orders['customer_first_name'] . ' monto $' . $orders['current_balance'] . ' duración: ' . $orders['duration'] . ' ' . $_frequency . ' interes: ' . $orders['interes_amount']);
					
					$this->loansquickbusiness_model->remove($id);
					$this->session->set_flashdata('msg','Datos eliminados exitosamente');
				}
	...
	{
		$this->loansquickbusiness_model->change_status($id,$mode);
		return redirect('loansquickbusiness');
	}
	...
	*/
	public function printTransactionDiv($order_id, $id_pay, $id_capital = "")
	{
		if($id_pay) {
			$transaction_pay = $this->loansquickbusiness_model->getTransactionByID($id_pay);
			$transaction_capital = $this->loansquickbusiness_model->getTransactionByID($id_capital);
			$order_data = $this->loansquickbusiness_model->getOrdersData($order_id);
			$loans_details = $this->loansquickbusiness_model->getLoansDetails($order_id, $order_data['customer_id']);
			...
			foreach ( $loans_details as $_recored ) {
				$original_loan_amount = $transaction['transactions_type'] == "CAPITAL" ? '<tr>
					<td class="text-left" colspan="2" border="1">CAPITAL PRESTADO</td>
					<td class="text-right" border="1"><table width="100%"><tr><td width="10%" class="text-left">+</td>
					<td width="90%" class="text-right">'.number_format(floatval($order_data['loans_amount']) + floatval($transaction_amount) - floatval($_recored->total_capital_paid), 2).'</td></tr></table></td>
			</tr>' : "";
				$additional_loan_amount = $transaction['transactions_type'] == "CAPITAL" && floatval($_recored->aditional_amount) != 0 ? '<tr>
					<td class="text-left" colspan="2" border="1">ADICIONALES</td>
					<td class="text-right" border="1"><table width="100%"><tr><td width="10%" class="text-left">+</td>
					<td width="90%" class="text-right">'.number_format($_recored->aditional_amount, 2).'</td></tr></table></td>
				</tr>' : "";
			...
			if ($transaction_capital['transactions_type'] == "CAPITAL") {
				$transaction_capital_detail = '<tr>
					...
					<td class="text-right" border="1"><table width="100%"><tr><td width="10%" class="text-left">'.($transaction['transactions_type'] != "CAPITAL" ? "" : "-").'</td>
					<td width="90%" class="text-right">'.number_format($transaction_capital_amount, 2).'</td></tr></table></td>
				</tr>';
			}
			$unpaid_loan_amount = $transaction['transactions_type'] == "CAPITAL" && floatval($_recored->unpaid_payments) != 0 ? '<tr>
					<td class="text-left" colspan="2" border="1">ATRASOS</td>
					<td class="text-right" border="1"><table width="100%"><tr><td width="10%" class="text-left">+</td>
					<td width="90%" class="text-right">'.number_format($_recored->unpaid_payments, 2).'</td></tr></table></td>
				</tr>' : "";
			$transaction_detail = '<tr>
				...
				<td class="text-right" border="1"><table width="100%"><tr><td width="10%" class="text-left">'.($transaction['transactions_type'] != "CAPITAL" ? "" : "-").'</td>
					<td width="90%" class="text-right">'.number_format($transaction_amount, 2).'</td></tr></table></td>
			</tr>';
			$current_loan_amount = $transaction_pay['transactions_type'] == "CAPITAL" ? '<tr>
				<td class="text-left" colspan="2" border="1">BALANCE</td>
				<td class="text-right" border="1"><table width="100%"><tr><td width="10%" class="text-left">=</td>
					<td width="90%" class="text-right">'.number_format(floatval($order_data['loans_amount']) + floatval($_recored->aditional_amount) + floatval($_recored->unpaid_payments) - floatval($_recored->total_capital_paid), 2).'</td></tr></table></td>
			</tr>' : "";
		}
			...
			<table width="100%" class="display" style="font-size:10px">
				<tr>
					<td width="68%">
						<div><span class="text-gray-light">CLIENTE PRESTAMO RAPIDO:</span><br>
				...
				<div id="invoice">
					<div class="invoice overflow-auto">
						<div style="min-width: 600px">
								...
								<main>
										<div class="row contacts">
											<div class="col-7 invoice-to">
												<div class="text-gray-light">CLIENTE PRESTAMO RAPIDO:</div>
										...
										<table border="0" cellspacing="0" cellpadding="0">
											...
											$total_capital_paid = $this->loansquickbusiness_model->countTotalPaidTransaction($order_id, $transaction['transactions_type']);
								...
								<a href="'.base_url().'loansquickbusiness/" class="text-center btn btn-outline-primary hidden-print noPrint">Regresar</a>
							</div>
						</div>
				</div>
			</div>
	...
	public function printPaymentReceipt($id, $order_id)
	{
		if($id) {
			...
			$paid_status = ($order_data['balance'] > 0) ? "A CRÉDITO" : "SALDADA";
							...
							<table class="table table-hover">
								...
								<tbody>
									...
									<tr>
										...
										<p><strong>'; $total_paid = $this->loansquickbusiness_model->countTotalPaidOrdersTransactionByDate($order_id); $html .= sprintf("%1\$.2F", $total_paid['amount']).'</strong></p></td>
									</tr>
									<tr>
	...
	*/
	...
	{
		if($id) {
			$order_data = $this->loansquickbusiness_model->getOrdersData($id);
			$company_info = $this->company_model->findOne($this->session->userdata('company_id'));
					...
					<div class="col-sm-6 pull pull-right">
						...
						<div class="table-responsive">
							...
							$html .= '<tr>
								...
								<tr>
								<th>BALANCE:</th>
								<td>'.sprintf("%1\$.2F", $order_data['balance']).'</td>
							</tr>                        
							</table>
						</div>
					</div>
	...
	*/
						...
						<div id="table">
							<table>
								...
								<tr class="tabletitle">
									...
									<td class="itemnumber"><h2>'.sprintf("%1\$.2F", $order_data['balance']).'</h2></td>
								</tr>
							</table>
						</div><!--End Table-->
	...
	public function printResponsiveInvoice($id)
	{
		if($id) {
			...
			<script src="//cdnjs.cloudflare.com/ajax/libs/jquery/3.2.1/jquery.min.js"></script>
			<style>
			#invoice{padding:10px 30px}.invoice{position:relative;background-color:#fff;min-height:380px;padding:5px}.invoice header{padding:10px 0;margin-bottom:10px;border-bottom:1px solid #3989c6}.invoice .company-details{text-align:left}.invoice .company-details .name{margin-top:0;margin-bottom:0}.invoice .contacts{margin-bottom:10px}.invoice .invoice-to{text-align:left}.invoice .invoice-to .to{font-size: x-large;margin-top:0;margin-bottom:0}.invoice .invoice-details{text-align:right}.invoice .invoice-details .invoice-id{font-size: xx-large;margin-top:0;color:#3989c6}.invoice main .thanks{margin-top:-100px;font-size:2em;margin-bottom:50px}.invoice main .notices{padding-left:6px;border-left:6px solid #3989c6}.invoice main .notices .notice{font-size:1.2em}.invoice table{width:100%;border-collapse:collapse;border-spacing:0;margin-bottom:10px}.invoice table td,.invoice table th{padding:5px;background:#eee;border-bottom:1px solid #fff}.invoice table th{white-space:nowrap;font-weight:400;font-size:16px}.invoice table td h3{margin:0;font-weight:400;color:#3989c6;font-size:1.2em}.invoice table .qty,.invoice table .total,.invoice table .unit{text-align:right;font-size:1.2em}.invoice table .no{color:#fff;font-size:1.6em;background:#3989c6}.invoice table .unit{background:#ddd}.invoice table .total{background:#3989c6;color:#fff}.invoice table tbody tr:last-child td{border:none}.invoice table tfoot td{background:0 0;border-bottom:none;white-space:nowrap;text-align:right;padding:5px 20px;font-size:1.2em;border-top:1px solid #aaa}.invoice table tfoot tr:first-child td{border-top:none}.invoice table tfoot tr:last-child td{color:#3989c6;border-top:1px solid #3989c6}.invoice table tfoot tr td:first-child{border:none}.invoice footer{width:100%;text-align:center;color:#777;padding:8px 0}@media print{.invoice{font-size:11px!important;overflow:hidden!important}.invoice footer{position:absolute;bottom:10px;page-break-after:always}.invoice>div:last-child{page-break-before:always}}
			</style>
			</head>
			<body onload="window.print();">
