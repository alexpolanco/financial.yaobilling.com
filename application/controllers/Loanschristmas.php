...
class Loanschristmas extends MY_Controller  { 
...
public function __construct()
	...
	{
		$data['recored'] = $this->loanschristmas_model->findAll();
		$this->load->view('admin/loanschristmas/loans-list',$data);
	}
	...
	public function pay()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			if (floatval($this->input->post('paid_amount')) > 0) {
				$next_transaction_id = $this->loanschristmas_model->getLastOrderTransactionId();
				$interes_amount = $this->loanschristmas_model->getOrdersData($id);
				...
				$this->logger->write( "Regalia", "agregado", 'Préstamo: '.$id.' Pago de cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('paid_amount') . ' ' . $interes_amount['interes_amount'] == str_replace(",","",$this->input->post('paid_amount')) ? "INTERES" : "PAGO IRREGULAR" );

				$create = $this->loanschristmas_model->pay($transaction_data, $accounting_data, $due_date, $id);	
			}
			...
			if (floatval($this->input->post('paid_payments_amount')) > 0) {
				...
				$createIrregular = $this->loanschristmas_model->pay($transaction_data, $accounting_data, $due_date, $id);
			}
			
			if (floatval($this->input->post('capitalpaid_amount') )> 0) {
				$next_transaction_id = $this->loanschristmas_model->getLastOrderTransactionId();
				$orders = $this->loanschristmas_model->getOrdersData($id);
				...
				$this->logger->write( "Regalia", "agregado", 'Préstamo: '.$id.' Abono al capital de cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('capitalpaid_amount') . ' ' );

				$createCapital = $this->loanschristmas_model->payCapital($capital_data, $order_data, $id);
			}
			...
			if($create == true || $createIrregular == true || $createCapital == true) {
				...
				$checkEndLoan = $this->loanschristmas_model->CheckEndLoan($id);
				if ($checkEndLoan) {
					$this->loanschristmas_model->updateCurrentBalance($id);
		...
		return redirect('loanschristmas');
	}
	...
	public function additional()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$this->logger->write( "Regalia", "agregado", 'Préstamo: '.$id.' Adicional al capital, cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('additional_amount') );

			$create = $this->loanschristmas_model->additional($transaction_data, $order_data, $id);
	...
	public function due()
	{
		...
		$data['recored'] = $this->loanschristmas_model->findAllDue();
		$this->load->view('admin/loanschristmas/loans-list',$data);
	}
	...
	public function receipt()
	{
		...
		$this->db->select("printed, con.due_date, loanschristmas.loans_no, con.payment_no, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, FORMAT(loanschristmas.loans_amount,2) loans_amount, FORMAT(loanschristmas.cuota,2) cuota, lt.loanstype_name, loanschristmas.entry_date, loanschristmas.start_date, loanschristmas.end_date, FORMAT(loanschristmas.due_amount,2) due_amount, FORMAT(con.interes,2) interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, a.routes_name,a.collector_id, IF(cob.collector_nickname='', cob.collector_first_name, CONCAT_WS(' ', cob.collector_first_name, '(', cob.collector_nickname, ')')) collector_first_name, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loanschristmas_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loanschristmas.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount");

		$this->db->join('customer cli', 'loanschristmas.customer_id=cli.customer_id', 'left');
		$this->db->join('customer cli2', 'loanschristmas.customer_id=cli2.customer_id', 'left');
		$this->db->join('routes a', 'loanschristmas.routes_id=a.routes_id', 'left');
		$this->db->join('collector cob', 'a.collector_id=cob.collector_id', 'left');
		$this->db->join('loanstype lt', 'loanschristmas.loanstype_id=lt.loanstype_id', 'left');
		$this->db->join('loanschristmas_accounting con', 'con.loans_no = loanschristmas.loans_no');
		...
		if(isset($customer_field) && $customer_field != ""){
			$this->db->where('loanschristmas.customer_id', $customer_field);
		}
		if(isset($routes_field) && $routes_field != ""){
			$this->db->where('loanschristmas.routes_id', $routes_field);
		}
		...
		$data['recored'] = $this->db->get('loanschristmas')->result();
		$this->load->view('admin/loanschristmas/loans-receipt',$data);
	}
	...
	public function receipt_due()
	{
		...
		$this->db->select("printed, con.due_date, loanschristmas.loans_no, (lt.loanstype_duration + 1) payment_no, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, FORMAT(loanschristmas.loans_amount,2) loans_amount, FORMAT(loanschristmas.cuota,2) cuota, lt.loanstype_name, loanschristmas.entry_date, loanschristmas.start_date, loanschristmas.end_date, FORMAT(loanschristmas.due_amount,2) due_amount, FORMAT(con.interes,2) interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, a.routes_name,a.collector_id, IF(cob.collector_nickname='', cob.collector_first_name, CONCAT_WS(' ', cob.collector_first_name, '(', cob.collector_nickname, ')')) collector_first_name, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loanschristmas_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loanschristmas.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount, '500.00' fee_payment");  
	...
	public function return_loans()
	{
		...
		$data['recored'] = $this->db->get('loanschristmas')->result();
		$this->load->view('admin/loanschristmas/loans-return-list',$data);
	}

	// order view
	public function loan_view()
	{
		$this->load->view('admin/loanschristmas/loan-view');
	}
	...
	public function pdf($segment_1,$segment_2,$segment_3="",$segment_4="")
	{
		if($segment_1 == "loan_1" || $segment_1 == "loan_letter" || $segment_1 == "loan_contract" || $segment_1 == "loan_receipt")
		{
			...
			else if($segment_1 == "loan_letter")
			{
				...
				loanschristmas.duration, loanschristmas.interes_amount, FORMAT(loanschristmas.loans_amount,2) loans_amount, FORMAT(loanschristmas.cuota,2) cuota, lt.loanstype_name, lt.loanstype_frequency, loanschristmas.contract_number, loanschristmas.contract_folio1, loanschristmas.contract_folio2, loanschristmas.entry_date, loanschristmas.entry_time, loanschristmas.start_date, loanschristmas.end_date, FORMAT(loanschristmas.due_amount,2) due_amount, loanschristmas.interes interes, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loanschristmas_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loanschristmas.loans_no) atraso, cli2.customer_address garante_address, cli2.customer_phone garante_phone ");
			
				$this->db->join('customer cli', 'loanschristmas.customer_id=cli.customer_id', 'left');
				$this->db->join('customer cli2', 'loanschristmas.customer_id=cli2.customer_id', 'left');
				$this->db->join('guarantor g1', 'loanschristmas.guarantor1_id=g1.guarantor_id', 'left');
				$this->db->join('guarantor g2', 'loanschristmas.guarantor2_id=g2.guarantor_id', 'left');
				//$this->db->join('routes a', 'loanschristmas.routes_id=a.routes_id', 'left');
				//$this->db->join('collector cob', 'a.collector_id=cob.collector_id', 'left');
				$this->db->join('loanstype lt', 'loanschristmas.loanstype_id=lt.loanstype_id', 'left');
				...
				$this->db->where('loanschristmas.company_id', $this->session->userdata('company_id'));			
				$this->db->where('loanschristmas.loans_no =', $segment_2);
						
				$this->db->order_by('customer_first_name', 'ASC');
				
				$data['order'] = $this->db->get('loanschristmas')->result();
				$data['date1'] = $segment_2;
				$data['date2'] = $segment_3;
				$this->load->view('admin/loanschristmas/'.$segment_1,$data);
			}
			...
			{
				$this->db->select("printed, con.due_date, loanschristmas.loans_no, loanschristmas.current_balance, tra.payment_date, tra.transactions_type, tra.amount transactions_amount,  con.payment_no, cli.customer_gender, cli.customer_nationality, cli.customer_civilstatus, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, loanschristmas.duration, loanschristmas.interes_amount, FORMAT(loanschristmas.loans_amount,2) loans_amount, FORMAT(loanschristmas.cuota,2) cuota, lt.loanstype_name, lt.loanstype_frequency, loanschristmas.contract_number, loanschristmas.contract_folio1, loanschristmas.contract_folio2, loanschristmas.entry_date, loanschristmas.entry_time, loanschristmas.start_date, loanschristmas.end_date, FORMAT(loanschristmas.due_amount,2) due_amount, loanschristmas.interes interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loanschristmas_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loanschristmas.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount");
				...
				$this->db->join('loanstype lt', 'loanschristmas.loanstype_id=lt.loanstype_id', 'left');
				$this->db->join('loanschristmas_accounting con', 'con.loans_no = loanschristmas.loans_no');
				$this->db->join('loanschristmas_transactions tra', 'tra.loans_no = loanschristmas.loans_no', 'left outer');
		...
		else
		{
			...
			$this->db->select("printed, con.due_date, loanschristmas.loans_no, con.payment_no, cli.customer_gender, cli.customer_nationality, cli.customer_civilstatus, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, FORMAT(loanschristmas.loans_amount,2) loans_amount, FORMAT(loanschristmas.cuota,2) cuota, lt.loanstype_name, loanschristmas.entry_date, loanschristmas.start_date, loanschristmas.end_date, FORMAT(loanschristmas.due_amount,2) due_amount, loanschristmas.interes interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, a.routes_name,a.collector_id, IF(cob.collector_nickname='', cob.collector_first_name, CONCAT_WS(' ', cob.collector_first_name, '(', cob.collector_nickname, ')')) collector_first_name, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT IFNULL(SUM(ct.cuota - ct.advance_amount), 0) FROM loanschristmas_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loanschristmas.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount");
	...
	{
		$data['recored'] = $this->loanschristmas_model->findAll();
		$this->load->view('admin/loanschristmas/loans-excel',$data);
	}
	...
	public function get_transactions_data($no)
	{
		...
		$query1 = $this->db->get('loanschristmas_accounting')->result();

		$this->db->where('company_id', $this->session->userdata('company_id'));
		$this->db->where('loans_no',$no);
		$query2 = $this->db->get('loanschristmas_transactions')->result();
	...
	public function get_loan_number($no)
	{
		$this->db->where('loanschristmas.company_id', $this->session->userdata('company_id'));
		$this->db->where('loans_no',$no);
		$data = $this->db->get('loanschristmas')->row_array();
	...
	public function create()
	{
		...
		{
			$this->load->view('admin/loanschristmas/loans-add');
		}
		else
		{
			$this->loanschristmas_model->insert();
	...
	public function loan_print($no = null)
	{
		...
		$this->db->join('loanschristmas_accounting la', 'loanschristmas.loans_no=la.loans_no', 'left');

		$this->db->where('loanschristmas.company_id', $this->session->userdata('company_id'));
		//$this->db->where('loanschristmas.loans_no',$no);

		$data['loanschristmas'] = $this->db->get('loanschristmas')->result();
		
		//$this->load->view('admin/loanschristmas/loan-pos-confirm',$data);
		//$this->load->view('admin/loanschristmas/loan-print-confirm',$data);
		$this->load->view('admin/loanschristmas/loan-report-confirm',$data);
	}
	...
	public function update($id)
	{
		...
		{
			$id = $this->input->post('loans_no');

			$this->loanschristmas_model->update($id);
			return redirect('loanschristmas');
		}
		else
		{
			...
			$this->db->update('loanschristmas_transactions',$data);

			$this->logger->write( "Regalia", "edicion", 'Transaccion: ' . $id . ' nuevo valor interes: ' . $_POST['edit_new_interes_amount'] . ' nuevo valor capital: ' . $_POST['edit_new_capital_amount'] );
		...
		else
		{
			...
			$data_ar = $this->loanschristmas_model->insert();
			//echo $data_ar;	
			//return redirect('loanschristmas/loan_print/'.$data_ar);
			return redirect('loanschristmas');
		}*/
	}
	...
	{		
		$data['recored'] = $this->loanschristmas_model->findOne($id);
		$this->load->view('admin/loanschristmas/loans-edit',$data);
	}
	...
	{		
		$data['recored'] = $this->loanschristmas_model->findOne($id);
		$this->load->view('admin/loanschristmas/loans-renew',$data);
	}
	...
	public function delete($id)
	{
		...
		{
			$rights = $this->check_rights();
			if(!in_array('loanschristmas/delete', $rights))
			{
			...
			else
			{
				if($id == "removeTransaction"){
					...
					$orders = $this->loanschristmas_model->getOrdersData($this->input->post('loan_no'));
					$transaction = $this->loanschristmas_model->getTransactionByID($this->input->post('transactions_no'));
					...
					$this->logger->write( "Regalia", "eliminado", 'Préstamo: '.$order_id.' Abono del cliente: ' . $orders['customer_first_name'] . ' monto ' . $transaction['transactions_type'] . ' $' . $transaction_amount );

					$this->loanschristmas_model->removeTransaction($this->input->post('loan_no'), $this->input->post('transactions_no'), $order_data);
					...
					$this->logger->write( "Regalia", "eliminado", 'Préstamo: '.$order_id.' Adicional del cliente: ' . $orders['customer_first_name'] . ' monto ' . $transaction['transactions_type'] . ' $' . $transaction_amount );

					$this->loanschristmas_model->removeAdditional($this->input->post('loan_no'), $this->input->post('transactions_no'), $order_data);					
					$this->session->set_flashdata('msg','Datos eliminados exitosamente');	
				}
				else{
					$orders = $this->loanschristmas_model->getOrdersData($id);
					...
					$this->logger->write( "Regalia", "eliminado", 'Préstamo: '.$orders['loans_no'].' Cliente: ' . $orders['customer_first_name'] . ' monto $' . $orders['current_balance'] . ' duración: ' . $orders['duration'] . ' ' . $_frequency . ' interes: ' . $orders['interes_amount']);
					
					$this->loanschristmas_model->remove($id);
					$this->session->set_flashdata('msg','Datos eliminados exitosamente');
				}
	...
	{
		$this->loanschristmas_model->change_status($id,$mode);
		return redirect('loanschristmas');
	}
	...
	*/
	public function printTransactionDiv($order_id, $id)
	{
		if($id) {
			...
			$order_data = $this->loanschristmas_model->getOrdersData($order_id);
			$loans_details = $this->loanschristmas_model->getLoansDetails($order_id, $order_data['customer_id']);
			...
			<table width="100%" class="display" style="font-size:10px">
				<tr>
					<td width="68%">
						<div><span class="text-gray-light">CLIENTE PRESTAMO REGALIA:</span><br>
	...
	public function printTransactionDivBackUp($order_id, $id)
	{
		if($id) {
			...
			$order_date = date("d-m-Y", strtotime($transaction['payment_date']) );
						...
						<div style="min-width: 600px">
								...
								<main>
										...
										<table border="0" cellspacing="0" cellpadding="0">
											...
											$total_capital_paid = $this->loanschristmas_model->countTotalPaidTransaction($order_id, $transaction['transactions_type']);
								...
								<a href="'.base_url().'loanschristmas/" class="text-center btn btn-outline-primary hidden-print noPrint">Regresar</a>
							</div>
						</div>
				</div>
			</div>
							...
							<table class="table table-hover">
								...
								<tbody>
									...
									<tr>
										...
										<p><strong>'; $total_paid = $this->loanschristmas_model->countTotalPaidOrdersTransactionByDate($order_id); $html .= sprintf("%1\$.2F", $total_paid['amount']).'</strong></p></td>
									</tr>
									<tr>
	...
	*/
	...
	{
		if($id) {
			$order_data = $this->loanschristmas_model->getOrdersData($id);
			$company_info = $this->company_model->findOne($this->session->userdata('company_id'));
