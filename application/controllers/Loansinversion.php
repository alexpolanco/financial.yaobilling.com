...
class Loansinversion extends MY_Controller  { 
...
public function __construct()
	...
	{
		$data['recored'] = $this->loansinversion_model->findAll();
		$this->load->view('admin/loansinversion/loans-list',$data);
	}
	...
	public function pay()
	{
		...
		$this->form_validation->set_rules('paid_amount_value', 'Cantidad a pagar', 'trim|required|numeric');
		$this->form_validation->set_rules('paid_concept', 'Concepto del pago', 'trim|required');
		//$this->form_validation->set_error_delimiters('<p class="text-danger">','</p>');

		$id = $this->input->post('txt_payloansno');
		$date = date('Y-m-d', strtotime($this->input->post('payer_date')));

		if ($this->form_validation->run() == TRUE) {
			$next_transaction_id = $this->loansinversion_model->getLastOrderTransactionId();
			$interes_amount = $this->loansinversion_model->getOrdersData($id);
			...
			$transaction_data = array(
				...
				'amount' => str_replace(",","",$this->input->post('paid_amount_value')),
				...
				'concept' => $this->input->post('paid_concept'),
				//'transaction_date' => $date,
				'user_id' => $this->session->userdata('userid'),
				'company_id' => $this->session->userdata('company_id')
			);

			$orders = $this->loansinversion_model->getOrdersData($id);
			$total_paid = $orders["due_amount"] + str_replace(",","",$this->input->post('paid_amount_value'));
			$balance = $orders["current_balance"] - str_replace(",","",$this->input->post('paid_amount_value'));
			...
			$this->logger->write( "Inversion", "agregado", 'Préstamo: '.$id.' Pago de cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('paid_amount_value') . ' ' . $this->input->post('paid_concept') );

			$create = $this->loansinversion_model->pay($transaction_data, $order_data, $id);
			...
			if (floatval($this->input->post('paid_payments_amount')) > 0) {
				...
				$createIrregular = $this->loansinversion_model->pay($transaction_data, $accounting_data, $due_date, $id);
			}
			
			if($create == true || $createIrregular == true) {
				$this->session->set_flashdata('msg','Datos insertados exitosamente');
				$this->printTransactionDiv($id, $transaction_data['transactions_no']);
				return redirect('loansinversion');
			}
			else {
	...
	public function additional()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			$this->logger->write( "Inversion", "agregado", 'Préstamo: '.$id.' Adicional al capital, cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('additional_amount') );

			$create = $this->loansinversion_model->additional($transaction_data, $order_data, $id);
	...
	public function due()
	{
		...
		$data['recored'] = $this->loansinversion_model->findAllDue();
		$this->load->view('admin/loansinversion/loans-list',$data);
	}
	...
	public function receipt()
	{
		...
		$this->db->select("printed, con.due_date, loansinversion.loans_no, con.payment_no, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, FORMAT(loansinversion.loans_amount,2) loans_amount, FORMAT(loansinversion.cuota,2) cuota, lt.loanstype_name, loansinversion.entry_date, loansinversion.start_date, loansinversion.end_date, FORMAT(loansinversion.due_amount,2) due_amount, FORMAT(con.interes,2) interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, a.routes_name,a.collector_id, IF(cob.collector_nickname='', cob.collector_first_name, CONCAT_WS(' ', cob.collector_first_name, '(', cob.collector_nickname, ')')) collector_first_name, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loansinversion_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loansinversion.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount");
		
		$this->db->join('customer cli', 'loansinversion.customer_id=cli.customer_id', 'left');
		$this->db->join('customer cli2', 'loansinversion.customer_id=cli2.customer_id', 'left');
		$this->db->join('routes a', 'loansinversion.routes_id=a.routes_id', 'left');
		$this->db->join('collector cob', 'a.collector_id=cob.collector_id', 'left');
		$this->db->join('loanstype lt', 'loansinversion.loanstype_id=lt.loanstype_id', 'left');
		$this->db->join('loansinversion_accounting con', 'con.loans_no = loansinversion.loans_no');
		...
		if(isset($customer_field) && $customer_field != ""){
			$this->db->where('loansinversion.customer_id', $customer_field);
		}
		if(isset($routes_field) && $routes_field != ""){
			$this->db->where('loansinversion.routes_id', $routes_field);
		}
		...
		$data['recored'] = $this->db->get('loansinversion')->result();
		$this->load->view('admin/loansinversion/loans-receipt',$data);
	}
	...
	public function receipt_due()
	{
		...
		$this->db->select("printed, con.due_date, loansinversion.loans_no, (lt.loanstype_duration + 1) payment_no, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, FORMAT(loansinversion.loans_amount,2) loans_amount, FORMAT(loansinversion.cuota,2) cuota, lt.loanstype_name, loansinversion.entry_date, loansinversion.start_date, loansinversion.end_date, FORMAT(loansinversion.due_amount,2) due_amount, FORMAT(con.interes,2) interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, a.routes_name,a.collector_id, IF(cob.collector_nickname='', cob.collector_first_name, CONCAT_WS(' ', cob.collector_first_name, '(', cob.collector_nickname, ')')) collector_first_name, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loansinversion_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loansinversion.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount, '500.00' fee_payment");  
	...
	public function return_loans()
	{
		...
		$data['recored'] = $this->db->get('loansinversion')->result();
		$this->load->view('admin/loansinversion/loans-return-list',$data);
	}

	// order view
	public function loan_view()
	{
		$this->load->view('admin/loansinversion/loan-view');
	}
	...
	public function pdf($segment_1,$segment_2,$segment_3="",$segment_4="")
	{
		if($segment_1 == "loan_1" || $segment_1 == "loan_letter" || $segment_1 == "loan_contract" || $segment_1 == "loan_receipt")
		{
			...
			else if($segment_1 == "loan_letter")
			{
				...
				loansinversion.duration, loansinversion.interes_amount, FORMAT(loansinversion.loans_amount,2) loans_amount, FORMAT(loansinversion.cuota,2) cuota, lt.loanstype_name, lt.loanstype_frequency, loansinversion.contract_number, loansinversion.contract_folio1, loansinversion.contract_folio2, loansinversion.entry_date, loansinversion.entry_time, loansinversion.start_date, loansinversion.end_date, FORMAT(loansinversion.due_amount,2) due_amount, loansinversion.interes interes, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loansinversion_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loansinversion.loans_no) atraso, cli2.customer_address garante_address, cli2.customer_phone garante_phone ");
			
				$this->db->join('customer cli', 'loansinversion.customer_id=cli.customer_id', 'left');
				$this->db->join('customer cli2', 'loansinversion.customer_id=cli2.customer_id', 'left');
				$this->db->join('guarantor g1', 'loansinversion.guarantor1_id=g1.guarantor_id', 'left');
				$this->db->join('guarantor g2', 'loansinversion.guarantor2_id=g2.guarantor_id', 'left');
				//$this->db->join('routes a', 'loansinversion.routes_id=a.routes_id', 'left');
				//$this->db->join('collector cob', 'a.collector_id=cob.collector_id', 'left');
				$this->db->join('loanstype lt', 'loansinversion.loanstype_id=lt.loanstype_id', 'left');
				...
				$this->db->where('loansinversion.company_id', $this->session->userdata('company_id'));			
				$this->db->where('loansinversion.loans_no =', $segment_2);
						
				$this->db->order_by('customer_first_name', 'ASC');
				
				$data['order'] = $this->db->get('loansinversion')->result();
				$data['date1'] = $segment_2;
				$data['date2'] = $segment_3;
				$this->load->view('admin/loansinversion/'.$segment_1,$data);
			}
			...
			{
				$this->db->select("printed, con.due_date, loansinversion.loans_no, loansinversion.current_balance, tra.payment_date, tra.transactions_type, tra.amount transactions_amount,  con.payment_no, cli.customer_gender, cli.customer_nationality, cli.customer_civilstatus, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, loansinversion.duration, loansinversion.interes_amount, FORMAT(loansinversion.loans_amount,2) loans_amount, FORMAT(loansinversion.cuota,2) cuota, lt.loanstype_name, lt.loanstype_frequency, loansinversion.contract_number, loansinversion.contract_folio1, loansinversion.contract_folio2, loansinversion.entry_date, loansinversion.entry_time, loansinversion.start_date, loansinversion.end_date, FORMAT(loansinversion.due_amount,2) due_amount, loansinversion.interes interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loansinversion_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loansinversion.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount");
				...
				$this->db->join('loanstype lt', 'loansinversion.loanstype_id=lt.loanstype_id', 'left');
				$this->db->join('loansinversion_accounting con', 'con.loans_no = loansinversion.loans_no');
				$this->db->join('loansinversion_transactions tra', 'tra.loans_no = loansinversion.loans_no', 'left outer');
		...
		else
		{
			...
			$this->db->select("printed, con.due_date, loansinversion.loans_no, con.payment_no, cli.customer_gender, cli.customer_nationality, cli.customer_civilstatus, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, FORMAT(loansinversion.loans_amount,2) loans_amount, FORMAT(loansinversion.cuota,2) cuota, lt.loanstype_name, loansinversion.entry_date, loansinversion.start_date, loansinversion.end_date, FORMAT(loansinversion.due_amount,2) due_amount, loansinversion.interes interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, a.routes_name,a.collector_id, IF(cob.collector_nickname='', cob.collector_first_name, CONCAT_WS(' ', cob.collector_first_name, '(', cob.collector_nickname, ')')) collector_first_name, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT IFNULL(SUM(ct.cuota - ct.advance_amount), 0) FROM loansinversion_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loansinversion.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount");
	...
	{
		$data['recored'] = $this->loansinversion_model->findAll();
		$this->load->view('admin/loansinversion/loans-excel',$data);
	}
	...
	public function get_transactions_data($no)
	{
		...
		$query1 = $this->db->get('loansinversion_accounting')->result();

		$this->db->where('company_id', $this->session->userdata('company_id'));
		$this->db->where('loans_no',$no);
		$query2 = $this->db->get('loansinversion_transactions')->result();
	...
	public function get_loan_number($no)
	{
		$this->db->where('loansinversion.company_id', $this->session->userdata('company_id'));
		$this->db->where('loans_no',$no);
		$data = $this->db->get('loansinversion')->row_array();
	...
	public function create()
	{
		...
		{
			$this->load->view('admin/loansinversion/loans-add');
		}
		else
		{
			$this->loansinversion_model->insert();
	...
	public function loan_print($no = null)
	{
		...
		$this->db->join('loansinversion_accounting la', 'loansinversion.loans_no=la.loans_no', 'left');

		$this->db->where('loansinversion.company_id', $this->session->userdata('company_id'));
		//$this->db->where('loansinversion.loans_no',$no);

		$data['loansinversion'] = $this->db->get('loansinversion')->result();
	
		//$this->load->view('admin/loansinversion/loan-pos-confirm',$data);
		//$this->load->view('admin/loansinversion/loan-print-confirm',$data);
		$this->load->view('admin/loansinversion/loan-report-confirm',$data);
	}
	...
	public function update($id)
	{
		...
		{
			$this->loansinversion_model->update($id);
			return redirect('loansinversion');
		}
		else
		{
			...
			$this->db->update('loansinversion_transactions',$data);

			$this->logger->write( "Inversion", "edicion", 'Transaccion: ' . $id . ' nuevo valor interes: ' . $_POST['edit_new_interes_amount'] . ' nuevo valor capital: ' . $_POST['edit_new_capital_amount'] );
		...
		else
		{
			...
			$data_ar = $this->loansinversion_model->insert();
			//echo $data_ar;	
			//return redirect('loansinversion/loan_print/'.$data_ar);
			return redirect('loansinversion');
		} */
	}
	...
	{		
		$data['recored'] = $this->loansinversion_model->findOne($id);
		$this->load->view('admin/loansinversion/loans-edit',$data);
	}
	...
	{		
		$data['recored'] = $this->loansinversion_model->findOne($id);
		$this->load->view('admin/loansinversion/loans-renew',$data);
	}
	...
	public function delete($id)
	{
		...
		{
			$rights = $this->check_rights();
			if(!in_array('loansinversion/delete', $rights))
			{
			...
			else
			{
				if($id == "removeTransaction"){
					...
					$orders = $this->loansinversion_model->getOrdersData($this->input->post('loan_no'));
					$transaction = $this->loansinversion_model->getTransactionByID($this->input->post('transactions_no'));
					...
					$this->logger->write( "Inversion", "eliminado", 'Préstamo: '.$order_id.' Abono del cliente: ' . $orders['customer_first_name'] . ' monto ' . $transaction['transactions_type'] . ' $' . $transaction_amount );

					$this->loansinversion_model->removeTransaction($this->input->post('loan_no'), $this->input->post('transactions_no'), $order_data);
					...
					$this->logger->write( "Inversion", "eliminado", 'Préstamo: '.$order_id.' Adicional del cliente: ' . $orders['customer_first_name'] . ' monto ' . $transaction['transactions_type'] . ' $' . $transaction_amount );

					$this->loansinversion_model->removeAdditional($this->input->post('loan_no'), $this->input->post('transactions_no'), $order_data);					
					$this->session->set_flashdata('msg','Datos eliminados exitosamente');	
				}
				else{
					$orders = $this->loansinversion_model->getOrdersData($id);
					...
					$this->logger->write( "Inversion", "eliminado", 'Préstamo: '.$orders['loans_no'].' Cliente: ' . $orders['customer_first_name'] . ' monto $' . $orders['current_balance'] . ' duración: ' . $orders['duration'] . ' ' . $_frequency . ' interes: ' . $orders['interes_amount']);
					
					$this->loansinversion_model->remove($id);
					$this->session->set_flashdata('msg','Datos eliminados exitosamente');
				}
	...
	{
		$this->loansinversion_model->change_status($id,$mode);
		return redirect('loansinversion');
	}
	...
	*/
	public function printTransactionDiv($order_id, $id)
	{
		if($id) {
			...
			$order_data = $this->loansinversion_model->getOrdersData($order_id);
			$loans_details = $this->loansinversion_model->getLoansDetails($order_id, $order_data['customer_id']);
			...
			<table width="100%" class="display" style="font-size:10px">
				<tr>
					<td width="68%">
						<div><span class="text-gray-light">CLIENTE PRESTAMO INVERSION:</span><br>
				...
				<div id="invoice">
					<div class="invoice overflow-auto">
						<div style="min-width: 600px">
								<header>
									<div class="row">
										<div class="col company-details">
											...
											<div>Fecha de la operación: '.$order_date.'</div>
										</div>
										<div class="col">
										...
										<table border="0" cellspacing="0" cellpadding="0">
											...
											$html .= sprintf("%1\$.2F", $discount).'</td>
												...
												<tr>
													...
													<td>';
											$total_paid = $this->loansinversion_model->countTotalPaidOrdersTransactionByDate($order_id);
								...
								<br>
									<a href="'.base_url().'loansinversion/" class="text-center btn btn-outline-primary hidden-print noPrint">Regresar</a>
							</div>
						</div>
				</div>
			</div>
							...
							<table class="table table-hover">
								...
								<tbody>
									...
									<tr>
										...
										<p><strong>'; $total_paid = $this->loansinversion_model->countTotalPaidOrdersTransactionByDate($order_id); $html .= sprintf("%1\$.2F", $total_paid['amount']).'</strong></p></td>
									</tr>
									<tr>
	...
	*/
	...
	{
		if($id) {
			$order_data = $this->loansinversion_model->getOrdersData($id);
			$company_info = $this->company_model->findOne($this->session->userdata('company_id'));
