...
class getloanschristmastransactions extends CI_Controller {
	...
	public function index(){
		...
		$query = $this->db->query('SELECT * FROM loanschristmas_transactions WHERE company_id='.$this->session->userdata('company_id').' AND amount != 0 AND (transactions_type="CAPITAL" OR transactions_type="INTERES" OR transactions_type="PAGO IRREGULAR") AND loans_no='.$id);
		...
		{
			$buttons = '';
			$buttons .= '<a target="__blank" href="'.base_url().'loanschristmas/printTransactionDiv/'.intval($row->loans_no).'/'.intval($row->transactions_no).'" class="btn btn-warning btn-default" data-toggle="tooltip" data-placement="bottom" title="Imprimir recibo"><i class="fa fa-print"></i></a>';
