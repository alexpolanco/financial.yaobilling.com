...
class backup_db extends CI_Controller {
	...
	public function index(){
		...
		function EXPORT_TABLES($host,$user,$pass,$name,  $tables=false, $backup_name=false ){
				...
				$zipfname = $name . "_" . date("Y-m-d_H-i-s").".zip";
				$zip = new ZipArchive();
				if($zip->open($zipfname,ZIPARCHIVE::CREATE))
				{
					$zip->addFromString($backup_name, $content);
					//$zip->addFile($backup_name,$backup_name);
					$zip->close();
				}
				...
				if (file_exists('./Backup/')) {
					copy($filename, './Backup/'.$filename) or die("Filed To Backup");
					unlink($filename);
		...
		EXPORT_TABLES($this->db->hostname,$this->db->username,$this->db->password,$this->db->database);
	}
}
