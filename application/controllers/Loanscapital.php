...
class Loanscapital extends MY_Controller  { 
...
public function __construct()
	...
	{
		/* $checkEndLoan = $this->loanscapital_model->CheckEndLoan(28);

		if ($checkEndLoan) {
			$this->pdf('loan_letter',28);
		}
		 */
		$data['recored'] = $this->loanscapital_model->findAll();
		$this->load->view('admin/loanscapital/loans-list',$data);
	}
	...
	*/
	public function pay()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			if (floatval($this->input->post('paid_amount')) > 0) {
				$next_transaction_id = $this->loanscapital_model->getLastOrderTransactionId();
				$interes_amount = $this->loanscapital_model->getOrdersData($id);
				...
				$transaction_data = array(
					...
					'transactions_type' => $interes_amount['interes_amount'] == str_replace(",","",$this->input->post('paid_amount')) ? "CUOTA" : "PAGO IRREGULAR",
					...
					'concept' => $interes_amount['interes_amount'] == str_replace(",","",$this->input->post('paid_amount')) ? "PAGO DE CUOTA" : "PAGO IRREGULAR",
				...
				$this->logger->write( "Capital", "agregado", 'Préstamo: '.$id.' Pago de cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('paid_amount') . ' ' . $interes_amount['interes_amount'] == str_replace(",","",$this->input->post('paid_amount')) ? "CUOTA" : "PAGO IRREGULAR" );

				$create = $this->loanscapital_model->pay($transaction_data, $accounting_data, $due_date, $id);
			}
			...
			if (floatval($this->input->post('paid_payments_amount')) > 0) {
				...
				$this->logger->write( "Capital", "agregado", 'Préstamo: '.$id.' Abono pago irregular - cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('paid_payments_amount') . ' ABONO PAGO IRREGULAR' );

				$createIrregular = $this->loanscapital_model->pay($transaction_data, $accounting_data, $due_date, $id);
			}

			if (floatval($this->input->post('capitalpaid_amount')) > 0) {
				$next_transaction_id = $this->loanscapital_model->getLastOrderTransactionId();
				$orders = $this->loanscapital_model->getOrdersData($id);
				...
				$total_paid = $orders["due_amount"] + str_replace(",","",$this->input->post('paid_amount'));
				$capitalpaid_amount = $orders["due_amount"] - str_replace(",","",$this->input->post('capitalpaid_amount'));
				...
				$this->logger->write( "Capital", "agregado", 'Préstamo: '.$id.' Abono al capital de cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('capitalpaid_amount') . ' ' );

				$createCapital = $this->loanscapital_model->payCapital($capital_data, $order_data, $id);
			}
			...
			if($create == true || $createIrregular == true || $createCapital == true) {
				...
				if($createCapital == true) {
					$this->printTransactionDiv($id, $capital_data['transactions_no']);
				}

				$checkEndLoan = $this->loanscapital_model->CheckEndLoan($id);
				if ($checkEndLoan) {
					$this->loanscapital_model->updateCurrentBalance($id);
	...
	public function additional()
	{
		...
		if ($this->form_validation->run() == TRUE) {
			...
			/*SELECT customer.customer_first_name cliente, loanscapital_transactions.payment_date fecha_pago, loanscapital.loans_no prestamo_no, loanscapital.due_amount monto_debido, loanscapital.current_balance balance FROM loanscapital 
LEFT JOIN loanscapital_transactions ON loanscapital.loans_no = loanscapital_transactions.loans_no 
LEFT JOIN customer ON customer.customer_id = loanscapital.customer_id 
WHERE due_amount>0 AND current_balance=0 
GROUP BY loanscapital.loans_no
ORDER BY loanscapital_transactions.payment_date DESC; */
			...
			$capital_to_paid = str_replace(",","",$this->input->post('new_capital_amount'));

			$order_data = array(
				//'paid_amount' => $total_paid), 
				'interes_amount' => $cuota,
				'interes' => $interes,
				'capital' => $capital_to_paid,
				'due_amount' => $due_amount,
				'additional_amount' => $additional_amount,
				'current_balance' => str_replace(",","",$balance)
			);

			$this->logger->write( "Capital", "agregado", 'Préstamo: '.$id.' Adicional al capital, cliente: ' . $this->input->post('payer_name') . ' $' . $this->input->post('additional_amount') );

			$create = $this->loanscapital_model->additional($transaction_data, $order_data, $id);
	...
	public function due()
	{
		...
		$data['recored'] = $this->loanscapital_model->findAllDue();
		$this->load->view('admin/loanscapital/loans-list',$data);
	}
	...
	public function receipt()
	{
		...
		$this->db->select("printed, con.due_date, loanscapital.loans_no, con.payment_no, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, FORMAT(loanscapital.loans_amount,2) loans_amount, FORMAT(loanscapital.cuota,2) cuota, lt.loanstype_name, loanscapital.entry_date, loanscapital.start_date, loanscapital.end_date, FORMAT(loanscapital.due_amount,2) due_amount, FORMAT(con.interes,2) interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, a.routes_name,a.collector_id, IF(cob.collector_nickname='', cob.collector_first_name, CONCAT_WS(' ', cob.collector_first_name, '(', cob.collector_nickname, ')')) collector_first_name, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loanscapital_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loanscapital.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount");
		
		$this->db->join('customer cli', 'loanscapital.customer_id=cli.customer_id', 'left');
		$this->db->join('customer cli2', 'loanscapital.customer_id=cli2.customer_id', 'left');
		$this->db->join('routes a', 'loanscapital.routes_id=a.routes_id', 'left');
		$this->db->join('collector cob', 'a.collector_id=cob.collector_id', 'left');
		$this->db->join('loanstype lt', 'loanscapital.loanstype_id=lt.loanstype_id', 'left');
		$this->db->join('loanscapital_accounting con', 'con.loans_no = loanscapital.loans_no');
		...
		if(isset($customer_field) && $customer_field != ""){
			$this->db->where('loanscapital.customer_id', $customer_field);
		}
		if(isset($routes_field) && $routes_field != ""){
			$this->db->where('loanscapital.routes_id', $routes_field);
		}
		...
		$data['recored'] = $this->db->get('loanscapital')->result();
		$this->load->view('admin/loanscapital/loans-receipt',$data);
	}
	...
	public function receipt_due()
	{
		...
		$this->db->select("printed, con.due_date, loanscapital.loans_no, (lt.loanstype_duration + 1) payment_no, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, FORMAT(loanscapital.loans_amount,2) loans_amount, FORMAT(loanscapital.cuota,2) cuota, lt.loanstype_name, loanscapital.entry_date, loanscapital.start_date, loanscapital.end_date, FORMAT(loanscapital.due_amount,2) due_amount, FORMAT(con.interes,2) interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, a.routes_name,a.collector_id, IF(cob.collector_nickname='', cob.collector_first_name, CONCAT_WS(' ', cob.collector_first_name, '(', cob.collector_nickname, ')')) collector_first_name, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loanscapital_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loanscapital.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount, '500.00' fee_payment");  
	...
	public function return_loans()
	{
		...
		$data['recored'] = $this->db->get('loanscapital')->result();
		$this->load->view('admin/loanscapital/loans-return-list',$data);
	}

	// order view
	public function loan_view()
	{
		$this->load->view('admin/loanscapital/loan-view');
	}
	...
	public function pdf($segment_1,$segment_2,$segment_3="",$segment_4="")
	{
		if($segment_1 == "loan_1" || $segment_1 == "loan_letter" || $segment_1 == "loan_contract" || $segment_1 == "loan_receipt")
		{
			...
			else if($segment_1 == "loan_letter")
			{
				...
				loanscapital.duration, loanscapital.interes_amount, FORMAT(loanscapital.loans_amount,2) loans_amount, FORMAT(loanscapital.cuota,2) cuota, lt.loanstype_name, lt.loanstype_frequency, loanscapital.contract_number, loanscapital.contract_folio1, loanscapital.contract_folio2, loanscapital.entry_date, loanscapital.entry_time, loanscapital.start_date, loanscapital.end_date, FORMAT(loanscapital.due_amount,2) due_amount, loanscapital.interes interes, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loanscapital_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loanscapital.loans_no) atraso, cli2.customer_address garante_address, cli2.customer_phone garante_phone ");
			
				$this->db->join('customer cli', 'loanscapital.customer_id=cli.customer_id', 'left');
				$this->db->join('customer cli2', 'loanscapital.customer_id=cli2.customer_id', 'left');
				$this->db->join('guarantor g1', 'loanscapital.guarantor1_id=g1.guarantor_id', 'left');
				$this->db->join('guarantor g2', 'loanscapital.guarantor2_id=g2.guarantor_id', 'left');
				//$this->db->join('routes a', 'loanscapital.routes_id=a.routes_id', 'left');
				//$this->db->join('collector cob', 'a.collector_id=cob.collector_id', 'left');
				$this->db->join('loanstype lt', 'loanscapital.loanstype_id=lt.loanstype_id', 'left');
				...
				$this->db->where('loanscapital.company_id', $this->session->userdata('company_id'));			
				$this->db->where('loanscapital.loans_no =', $segment_2);
						
				$this->db->order_by('customer_first_name', 'ASC');
				
				$data['order'] = $this->db->get('loanscapital')->result();
				$data['date1'] = $segment_2;
				$data['date2'] = $segment_3;
				$this->load->view('admin/loanscapital/'.$segment_1,$data);
			}
			...
			{
				$this->db->select("printed, con.due_date, loanscapital.loans_no, loanscapital.current_balance, tra.payment_date, tra.transactions_type, tra.amount transactions_amount,  con.payment_no, cli.customer_gender, cli.customer_nationality, cli.customer_civilstatus, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, loanscapital.duration, loanscapital.interes_amount, FORMAT(loanscapital.loans_amount,2) loans_amount, FORMAT(loanscapital.cuota,2) cuota, lt.loanstype_name, lt.loanstype_frequency, loanscapital.contract_number, loanscapital.contract_folio1, loanscapital.contract_folio2, loanscapital.entry_date, loanscapital.entry_time, loanscapital.start_date, loanscapital.end_date, FORMAT(loanscapital.due_amount,2) due_amount, loanscapital.interes interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT FORMAT(IFNULL(SUM(ct.cuota - ct.advance_amount), 0),2) FROM loanscapital_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loanscapital.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount");
				...
				$this->db->join('loanstype lt', 'loanscapital.loanstype_id=lt.loanstype_id', 'left');
				$this->db->join('loanscapital_accounting con', 'con.loans_no = loanscapital.loans_no');
				$this->db->join('loanscapital_transactions tra', 'tra.loans_no = loanscapital.loans_no', 'left outer');
		...
		else
		{
			...
			$this->db->select("printed, con.due_date, loanscapital.loans_no, con.payment_no, If(cli.customer_nickname ='', cli.customer_first_name, CONCAT_WS(' ', cli.customer_first_name, '(', cli.customer_nickname, ')')) AS customer_first_name,  cli.customer_id, cli.customer_personalid, cli.customer_address, cli.customer_phone, cli.customer_workplace, cli.customer_occupation, FORMAT(loanscapital.loans_amount,2) loans_amount, FORMAT(loanscapital.cuota,2) cuota, lt.loanstype_name, loanscapital.entry_date, loanscapital.start_date, loanscapital.end_date, FORMAT(loanscapital.due_amount,2) due_amount, FORMAT(con.interes,2) interes, FORMAT(con.capital,2) capital, FORMAT(con.payment,2) payment, a.routes_name,a.collector_id, IF(cob.collector_nickname='', cob.collector_first_name, CONCAT_WS(' ', cob.collector_first_name, '(', cob.collector_nickname, ')')) collector_first_name, IF(cli2.customer_nickname='', cli2.customer_first_name, CONCAT_WS(' ', cli2.customer_first_name, '(', cli2.customer_nickname, ')')) AS garante, (SELECT IFNULL(SUM(ct.cuota - ct.advance_amount), 0) FROM loanscapital_accounting ct WHERE ct.due_date < CURDATE() AND ct.printed='NO' AND loans_no=loanscapital.loans_no) atraso, FORMAT(con.balance,2) balance, cli2.customer_address garante_address, cli2.customer_phone garante_phone, FORMAT(con.advance_amount,2) advance_amount");
	...
	{
		$data['recored'] = $this->loanscapital_model->findAll();
		$this->load->view('admin/loanscapital/loans-excel',$data);
	}
	...
	public function get_transactions_data($no)
	{
		...
		$query1 = $this->db->get('loanscapital_accounting')->result();

		$this->db->where('company_id', $this->session->userdata('company_id'));
		$this->db->where('loans_no',$no);
		$query2 = $this->db->get('loanscapital_transactions')->result();
	...
	public function get_loan_number($no)
	{
		$this->db->where('loanscapital.company_id', $this->session->userdata('company_id'));
		$this->db->where('loans_no',$no);
		$data = $this->db->get('loanscapital')->row_array();
	...
	public function create()
	{
		...
		{
			$this->load->view('admin/loanscapital/loans-add');
		}
		else
		{
			$this->loanscapital_model->insert();
	...
	public function loan_print($no = null)
	{
		...
		$this->db->join('loanscapital_accounting la', 'loanscapital.loans_no=la.loans_no', 'left');

		$this->db->where('loanscapital.company_id', $this->session->userdata('company_id'));
		//$this->db->where('loanscapital.loans_no',$no);

		$data['loanscapital'] = $this->db->get('loanscapital')->result();
		
		//$this->load->view('admin/loanscapital/loan-pos-confirm',$data);
		//$this->load->view('admin/loanscapital/loan-print-confirm',$data);
		$this->load->view('admin/loanscapital/loan-report-confirm',$data);
	}
	...
	public function update($id)
	{
		...
		{
			$this->loanscapital_model->update($id);
			return redirect('loanscapital');
		}
		else
		{
			...
			$this->db->update('loanscapital_transactions',$data);

			$this->logger->write( "Capital", "edicion", 'Transaccion: ' . $id . ' nuevo valor interes: ' . $_POST['edit_new_interes_amount'] . ' nuevo valor capital: ' . $_POST['edit_new_capital_amount'] );
		...
		else
		{
			...
			$data_ar = $this->loanscapital_model->insert();
			//echo $data_ar;	
			//return redirect('loanscapital/loan_print/'.$data_ar);
			return redirect('loanscapital');
		} */
	}
	...
	{		
		$data['recored'] = $this->loanscapital_model->findOne($id);
		$this->load->view('admin/loanscapital/loans-edit',$data);
	}
	...
	{		
		$data['recored'] = $this->loanscapital_model->findOne($id);
		$this->load->view('admin/loanscapital/loans-renew',$data);
	}
	...
	public function delete($id)
	{
		...
		{
			$rights = $this->check_rights();
			if(!in_array('loanscapital/delete', $rights))
			{
			...
			else
			{
				if($id == "removeTransaction"){
					...
					$orders = $this->loanscapital_model->getOrdersData($this->input->post('loan_no'));
					$transaction = $this->loanscapital_model->getTransactionByID($this->input->post('transactions_no'));
					...
					$this->logger->write( "Capital", "eliminado", 'Préstamo: '.$order_id.' Abono del cliente: ' . $orders['customer_first_name'] . ' monto ' . $transaction['transactions_type'] . ' $' . $transaction_amount );
					
					$this->loanscapital_model->removeTransaction($this->input->post('loan_no'), $this->input->post('transactions_no'), $order_data);
					...
					$this->logger->write( "Capital", "eliminado", 'Préstamo: '.$order_id.' Adicional del cliente: ' . $orders['customer_first_name'] . ' monto ' . $transaction['transactions_type'] . ' $' . $transaction_amount );

					$this->loanscapital_model->removeAdditional($this->input->post('loan_no'), $this->input->post('transactions_no'), $order_data);					
					$this->session->set_flashdata('msg','Datos eliminados exitosamente');	
				}
				else{
					$orders = $this->loanscapital_model->getOrdersData($id);
					...
					$this->logger->write( "Capital", "eliminado", 'Préstamo: '.$orders['loans_no'].' Cliente: ' . $orders['customer_first_name'] . ' monto $' . $orders['current_balance'] . ' duración: ' . $orders['duration'] . ' ' . $_frequency . ' interes: ' . $orders['interes_amount']);
					
					$this->loanscapital_model->remove($id);
					$this->session->set_flashdata('msg','Datos eliminados exitosamente');
				}
	...
	{
		$this->loanscapital_model->change_status($id,$mode);
		return redirect('loanscapital');
	}
	...
	*/
	public function printTransactionDiv($order_id, $id)
	{
		if($id) {
			...
			$order_data = $this->loanscapital_model->getOrdersData($order_id);
			$loans_details = $this->loanscapital_model->getLoansDetails($order_id, $order_data['customer_id']);
			...
			$id_pad = str_pad($transaction['transactions_no'], 8, "0", STR_PAD_LEFT);
			...
			$transaction_type = $transaction['transactions_type'] == "ADICIONAL" ? "ADICIONAL" : "PAGO";
			...
			<table width="100%" class="display" style="font-size:10px">
				<tr>
					<td width="68%">
						<div><span class="text-gray-light">CLIENTE PRESTAMO INTERES Y CAPITAL:</span><br>
			...
			<div class="thanks" style="font-size:10px;text-align:center">{$company_info['company_slogan']}</div>
			EOF;			
	...
	public function printTransactionDivBackUp($order_id, $id)
	{
		if($id) {
			...
			$order_date = date("d-m-Y", strtotime($order_data['payment_date']) );
			$transaction_date = date("d-m-Y", strtotime($transaction['accounting_due_date']) );
						...
						<div style="min-width: 600px">
								...
								<main>
										...
										$html .= '
												...
												<div class="date">Recibo No.: '.str_pad($id, 8, "0", STR_PAD_LEFT).' Fecha: '.$transaction_date.'</div>
										...
										<table border="0" cellspacing="0" cellpadding="0">
											...
											<tbody>
												<tr>
														<td class="text-left"><h3>'.$transaction['concept'].'</h3></td>
														<td class="total">'.sprintf("%1\$.2F", $transaction['amount']).'</td>
												</tr>
											</tbody>
											<tfoot>';
											/*
											...
											$html .= sprintf("%1\$.2F", $discount).'</td>
												...
												<tr>
													...
													<td>';
											$total_paid = $this->loanscapital_model->countTotalPaidOrdersTransactionByDate($order_id);
												$html .= sprintf("%1\$.2F", $total_paid['amount']);
												$html .= '</td>
												</tr>';
									...
									<a href="'.base_url().'loanscapital/" class="text-center btn btn-outline-primary hidden-print noPrint">Regresar</a>
							</div>
						</div>
				</div>
			</div>
							...
							<table class="table table-hover">
								...
								<tbody>
									...
									<tr>
										...
										<p><strong>'; $total_paid = $this->loanscapital_model->countTotalPaidOrdersTransactionByDate($order_id); $html .= sprintf("%1\$.2F", $total_paid['amount']).'</strong></p></td>
									</tr>
									<tr>
	...
	*/
	...
	{
		if($id) {
			$order_data = $this->loanscapital_model->getOrdersData($id);
			$company_info = $this->company_model->findOne($this->session->userdata('company_id'));
